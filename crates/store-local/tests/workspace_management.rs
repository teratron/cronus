...
use cronus_store_local::workspace::{WorkspaceId, WorkspaceManager, WorkspaceTemplate};
...
fn duplicate_create_returns_already_exists() {
    ...
    assert!(
        matches!(
            err,
            Err(cronus_store_local::workspace::WorkspaceError::AlreadyExists(_))
        ),
...
fn set_active_nonexistent_fails() {
    ...
    assert!(
        matches!(
            m.set_active(&id),
            Err(cronus_store_local::workspace::WorkspaceError::NotFound(_))
        ),

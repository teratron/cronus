...
*Core-decomposition wave (added v2.14.0). The two entries below promote together: reviewing either alone leaves the source-layout §4.4 delegation dangling.*

- [ ] **Crate Topology (Core Decomposition)** ([l2-crate-topology.md](specifications/l2-crate-topology.md)) [L2] — `RFC` — `Implements: l1-architecture.md`; partitions the ~20k-line, 53-module `core` crate along **dependency weight and provider seams**, not domain area, turning INV-8's inward-dependency convention from something the compiler cannot see into a boundary it enforces. Measured basis: only 11 inter-module edges across 53 modules; 6 infrastructure modules (18% of lines) hold every external dependency; and a single field — `context_router` holding a concrete `&MemoryStore` and calling one method on it — chains the entire domain to `rusqlite`, which is why the TUI pulls 152 transitive crates to use a 37-line redaction helper. Also the precondition for `l1-deployment-neutrality` DN-2/DN-3: the identity / auth / user-data provider seams do not exist today, so "the base binary and the hosted binary are the same code with different providers" is currently unrealizable. Backlog until reviewed to Stable
- [ ] **Source Layout — 1.2.0 delta only** ([l2-source-layout.md](specifications/l2-source-layout.md)) [L2] — `RFC` — **the spec is `[x]` Done in Phase 1 at its delivered 1.1.0 scope; do not move that entry here.** Only the 1.2.0 amendment is parked: it resolves the §4.4 crate-granularity TBD by delegating to `l2-crate-topology`, adds §4.5 recording the decision, and marks §4.1/§4.2 as pre-migration state. The amendment rule (RULES §2) demoted the file `Stable → RFC` pending review of the topology spec — the status reflects the un-reviewed delta, not the delivered layout. Same shape as the KAN-8 delta over Done Phase 5
...
- [ ] **Service Activation** ([l2-service-activation.md](specifications/l2-service-activation.md)) [L2] — `RFC` — `Implements: l1-background-activation.md`; the per-OS realization (Windows / macOS / Linux) of the three activation modes. Two constraints drive every choice: the background engine **runs as the user, never as a superuser** — a `LocalSystem` service or a `root` daemon has a different profile and would silently operate on a different, empty state root, which eliminates the textbook Windows Service and root daemon and is what makes BA-11 hold across a mode switch — and elevation authorizes **registration only, never execution** (BA-6). Presumes the platform-adapter crate seam of `l2-crate-topology` §4.4: a soft dependency, not a hard `Implements:` edge, but BA-4's structural guarantee has no crate to live in until that seam exists. Open: Windows S4U spike, non-systemd Linux hosts, `tauri-plugin-autostart` dependency decision. Backlog until reviewed to Stable

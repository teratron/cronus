...
Tactical registry of all phases and their statuses, ordered by the growth metaphor (seed → stem → leaf → flower). Atomic checklists live in `tasks/phase-{N}.md`. Phase 7 (TUI) is Done (Archived); Phase 8 (Flower — Desktop App) is now decomposed and ready to run; later phases are decomposed on entry.
...
| [Phase 8](tasks/phase-8.md) | Flower — Desktop App: application UI, office view, dashboard | `In Progress` |

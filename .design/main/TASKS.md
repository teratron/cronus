...
| [Phase 15](tasks/phase-15.md) | Memory Capture Policy & Metadata (L2): the MI-6/10/11/12 capture-path delta deferred at Phase 14 — salience-gated capture policy + actor/expiry/subject/cross-ref metadata (MI-6), capture-time date normalization (MI-10), caller capture directives (MI-11), raw/inferred mode (MI-12), all behind the UserDataStore seam → 5 tasks / 3 tracks (A capture-metadata schema · B capture path · T) | `Todo` |

...
- [l1-extensions.md](l1-extensions.md) — external-service action nodes use the extension/MCP boundary; the polling/push transport behind an `external_event` trigger (AP-16 change classification)
- [l1-context-provenance.md](l1-context-provenance.md) — external content reaching a change-detector (AP-16) is external-authored and untrusted-by-default (CP-1); it crosses the neutralization boundary before an action node acts on it
...
- **AP-16 External-source change classification**: a change-detector over an *external, uncontrolled* source (a fetched page, a feed, an API poll result reaching the office through an `external_event` trigger, §4.2) MUST assess change against a **declared canonical projection** of the content — not its raw bytes. Normalization removes declared volatile/cosmetic regions (whitespace and reflow, rotating embeds, timestamps, session nonces), so a source that merely re-rendered without changing meaning is classified `same`, not `changed`. Each observation is classified into a **typed change status** — `new` (no retained prior for this source + comparison tag), `same` (canonical form equals the retained prior), `changed` (canonical form differs), or `removed` (the source became durably unavailable or empty) — carried as the detector's output, not a bare differ-boolean. `removed` is first-class and distinct from both `same` and a transient fetch **error**: a durably vanished dependency is an actionable event (work grounded on it is now unbacked), never silently swallowed as "no change" nor retried forever as an error. A comparison lineage MAY be namespaced by a **tag** so one source is watched under independent baselines. This hardens AP-8 change detection — which for *internal* state (a counter, a Kanban state) correctly compares raw values, there being no cosmetic churn — for the external-content case where raw equality is noise-dominated; it reuses AP-10 retained-prior continuity and the AP-14 durable baseline (which stores the canonical form), and because the retained prior is required, a zero-retention posture degrades to unclassified re-emit, never a false `same`. External content crossing a change-detector is external-authored and untrusted-by-default (`l1-context-provenance.md` CP-1) and passes the neutralization boundary before any `action` node acts on it.
...
Node memory is a schema-bounded map, office-scoped (AP-7), individually resettable, and bound by the same content exclusions as event payloads (AP-4). It is **not** the office memory store (`l1-memory-model.md`): it is pipeline-local working state with no cross-office recall and no semantic indexing. A node that declares no memory is guaranteed stateless and freely retryable. §4.12 generalizes this to a scope hierarchy (node-private vs pipeline-shared) with a declared volatile-vs-durable persistence backend (AP-14). The change-detection row above assumes raw last-seen equality, which is correct for internal state; for an *external, uncontrolled* source that equality is noise-dominated, and §4.13 (AP-16) hardens it with a canonical projection and a typed change status.
...
### 4.13 External-Source Change Classification (AP-16)

AP-8 change detection compares a node's last-seen value and emits when it *differs*. For an **internal** signal — a counter, a threshold, a Kanban state — raw equality is exactly right: internal state has no cosmetic churn, so "differs" means "changed." For an **external, uncontrolled** source arriving through an `external_event` trigger (§4.2) — a fetched page, a feed, an API poll result — raw equality is the wrong test. External content re-renders constantly without changing meaning: whitespace and reflow shift, embeds and ads rotate, timestamps and session nonces refresh every poll. A byte-comparison flags every one of those as a change, and the trigger fires on noise forever. AP-16 hardens the external-content case:

```text
[REFERENCE]
classify(observation, retained_prior, volatile_spec):
    if observation.unavailable:          return removed   // durable 404 / empty — NOT a transient error
    canon := normalize(observation.content, volatile_spec) // strip declared volatile/cosmetic regions
    if retained_prior is None:           return new        // first sighting of this source+tag
    if canon == retained_prior.canon:    return same        // re-rendered, not re-authored
    return changed                                          // canonical form actually differs
// a transient fetch failure is classified error (retry), distinct from removed and from same
```

Two disciplines make it work. **Canonical projection:** change is assessed against a *normalized* form that removes declared volatile/cosmetic regions, so a source that merely re-rendered is classified `same`, not `changed`. The durable baseline (AP-14) stores the canonical form, not the raw content, and `normalize` is a pure, deterministic function, so the classification is replayable. **Typed status, not a differ-boolean:** each observation is one of `new` / `same` / `changed` / `removed`, carried as the detector's output. `removed` — the source became durably unavailable or empty — is first-class and distinct from both `same` and a transient fetch `error`: a vanished dependency is an actionable event (work grounded on that source is now unbacked), never silently swallowed as "no change" nor retried indefinitely as an error. A comparison lineage MAY be namespaced by a **tag**, so one source can be watched under independent baselines (e.g. a full-content baseline and a single-section baseline). The mechanic depends on a retained prior (AP-10 continuity); under a zero-retention posture it degrades to unclassified re-emit — never a false `same`.

Semantic **materiality** — "this page changed, but does the change matter for my goal?" — is deliberately *not* a new mechanic here: it is the existing `filter` node holding a reference-free judgment (`l1-evaluation-suites.md`) over the `changed` payload, dropping immaterial deltas before they reach an `action`. AP-16 supplies the trustworthy `changed` signal; composition supplies the relevance gate.
...
| 1.4.0 | 2026-07-23 | Core Team | Added AP-16 (external-source change classification) + §4.13 — a change-detector over an *external, uncontrolled* source (page/feed/API poll reaching the office via an `external_event` trigger) assesses change against a **declared canonical projection** (normalization strips declared volatile/cosmetic regions — whitespace/reflow, rotating embeds, timestamps, nonces — so a re-rendered-but-unchanged source is `same`, not `changed`) and emits a **typed change status** {`new`, `same`, `changed`, `removed`} rather than a differ-boolean, with `removed` (durable disappearance) first-class and distinct from both `same` and a transient fetch `error`; comparison lineage MAY be tag-namespaced; the durable baseline (AP-14) stores the canonical form, `normalize` is deterministic (replayable), and a zero-retention posture degrades to unclassified re-emit, never a false `same`. Hardens AP-8 (raw last-seen equality — correct for *internal* state, noise-dominated for external content) reusing AP-10 continuity + AP-14 durable baseline; semantic materiality gating is deliberately left as `filter` + reference-free judgment (l1-evaluation-suites) composition, not a new mechanic. Added l1-context-provenance Related (external content untrusted-by-default, CP-1 neutralization before an action node). Additive — L1 stays Stable (C9). Distilled from a study of an external web-data-acquisition engine's re-scrape change-tracking (canonical-form diff + four-way change status) and scheduled-monitor check summary (same/changed/new/removed/error as separate outcomes) — the one delta beyond Cronus's existing external_event polling + AP-8 change detection + scheduler cadence + trigger-triage reaction. Concept-only. |

...
- **IE-9 (State the engine cannot see is invalidated explicitly, with a declared scope, and the prior output survives until its replacement resolves):** `[ADDED v1.1.0]` where a step's result depends on state **held by its supplier and invisible to the engine's input identity** — a contributed component's own mode, a toggled presentation, a fold, an operator preference the declaration does not carry — the supplier MUST be able to **invalidate its own memoized results explicitly**, and that invalidation MUST carry a **scope**: the whole derivation, or one identified subject within it. Three rules follow, and each closes a distinct failure:

  1. **Explicit beats inferred.** The engine treats the derivation as a pure function of its declared inputs (IE-4) and is correct to do so; a supplier holding hidden state therefore has *nothing the engine can observe changing*, and would otherwise keep serving its first answer forever. Widening the key to "anything the supplier might hold" is the alternative, and it defeats memoization entirely.
  2. **Scope is declared, not assumed whole.** Invalidating everything because one subject changed re-runs third-party work for subjects that did not change — expensive and, where the derivation is nondeterministic in wall-clock terms, visibly disruptive. An invalidation naming a subject that no longer exists invalidates nothing and is not an error: identities race the graph.
  3. **The prior output stays live until the replacement resolves.** An invalidation MUST NOT drop the current result and expose a hole while the recomputation runs; the previous output remains in effect until a new one is accepted, and a recomputation that declines, fails, or times out degrades exactly like a first-time failure would — never into a flicker through an empty or fallback state that the consumer had already moved past.

  This is the imperative counterpart to IE-5's declared freshness policy: IE-5 is a standing property of a step, IE-9 is an event raised by the party that knows something changed. IE-6 is unaffected — invalidation is still identity-and-event driven, never a timer that quietly serves stale output.
...
| --- | --- | --- | --- |
| 1.1.0 | 2026-08-13 | Core Team | Added IE-9 (explicit, scoped invalidation for engine-invisible state, with retention until replacement) — IE-1…IE-8 assume everything that can change a result is in the declared input identity, which holds for the engine's own graph and fails for a *supplier-held* state the declaration cannot carry (a contributed component's mode, a toggled presentation, a per-subject fold). Such a supplier has nothing the engine can observe changing and would serve its first answer forever, while widening the key to cover it defeats memoization outright — so the supplier raises an **explicit** invalidation carrying a **declared scope** (whole derivation, or one identified subject), an invalidation naming a vanished subject is a no-op rather than an error since identities race the graph, and the **prior output stays in effect until its replacement is accepted** so an invalidation never opens a hole or flickers through a fallback the consumer had moved past; a declined, failed, or timed-out recomputation degrades exactly like a first-time failure. The imperative counterpart to IE-5's standing freshness policy, leaving IE-6 intact (identity-and-event driven, never a timer). |

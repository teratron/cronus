...
- **REA-12 (Absent, present-but-unconfigured, and present-but-unreachable are three verdicts — and a declaration marker is none of them):** [ADDED v1.2.0] REA-11 establishes reachability per (capability, executor) pair and in the mode of use; this invariant states what the probe must actually return. A capability resolves to one of **absent** (not installed, not provided — the actor chose not to have it, and skipping it is correct and quiet), **present-but-unconfigured** (installed and non-functional for want of a setup step — a **defect**, reported loudly, naming the step), or **present-and-reachable** (exercised, this run, through the interface that will be used). Collapsing the first two produces the two symmetric failures: nagging about a capability the actor declined, or silently skipping a safeguard the actor believes is in force. Two evidence rules bind the probe. An **environment marker is not a capability**: an inherited variable, a declared setting, or a recorded feature flag proves only that the declaration was propagated, and a sandbox that passes the declaration through while blocking the resource it names satisfies every marker check and then fails inside the work. And an **artifact left behind is not a live owner**: a socket, lock file, or registration outliving the process that created it proves only that the process once existed, so a reachability probe asks the thing itself and treats a stale artifact as *absent owner*, never as presence — the local-probe sibling of RLV-2's evidence-sourced-from-the-owning-host rule.
...
| 1.2.0 | 2026-09-04 | Core Team | Added REA-12 — absent, present-but-unconfigured, and present-but-unreachable are three verdicts, and a declaration marker is none of them. REA-11 already required a capability to be *exercised in the mode it will be used*; REA-12 states what the probe returns and what evidence it may accept. The three-verdict split is load-bearing in both directions: absence is a choice and is skipped quietly, while installed-but-unconfigured is a **defect** reported loudly with the missing step named — collapsing them either nags about a declined capability or silently skips a safeguard the actor believes is in force (the shape is visible wherever a pre-change snapshot step is skipped in silence when its tool is absent but fails loudly when the tool is present and unconfigured). Two evidence rules: an inherited environment marker proves only that a declaration propagated, not that the resource it names is reachable — a sandbox passing the variable through while blocking the socket clears every marker check and then aborts inside the work; and an artifact outliving its owner (a socket, lock, or registration left by a dead process) is *absent owner*, never presence — the local sibling of RLV-2. |

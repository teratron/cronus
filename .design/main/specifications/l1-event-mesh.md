...
- **EM-11 (A record-attached effect travels as data, never as a callback into the emitter):** [ADDED v1.1.0] where a durable record carries an action a consumer may later invoke — a notification whose click does something, a queued item with a completion, a surfaced alert with a remedy — that action is carried **as data alongside the record** and executed by whoever holds the record, never as a live callback registered back into the emitting process. A callback binds the emitter's lifetime to the consumer's: the emitter must stay alive waiting to be called, and it dies unanswered whenever the consuming surface restarts underneath it — which is routine, and which is why the emitters most likely to attach an action (one-shot commands, installers, automations that restart the very surface they notified) are exactly the ones a callback cannot serve. Carrying the effect as data lets the emitter exit immediately and lets a **record restored after a restart behave exactly like a live one**. Three rules bind the carried form. It is a **pre-resolved argument vector, never a string re-parsed at execution time**, so a value inside it that came from elsewhere — a filename, a title, a foreign process's name — is one argument and can never become a command (composing EM-1's typed envelope and CP-1's untrusted-by-default). Any resource the record references and the emitter may reclaim (an image, a temporary file, a context snapshot) is **copied into the record**, not pointed at, since a durable record referencing a transient original is a record that decays. And the surface accepting such a record admits an **enumerated set of fields with unknown ones rejected**, never a general pass-through, because the field carrying the effect is the only door to execution and a pass-through is a second one.
...
| 1.1.0 | 2026-09-04 | Core Team | Added EM-11 — a record-attached effect travels as data, never as a callback into the emitter. EM-1…EM-10 govern events in flight; a durable record that carries an action a consumer may invoke later (a notification whose click does something, a surfaced alert with a remedy) was unspecified, and the natural implementation — register a callback and wait to be called — binds the emitter's lifetime to the consumer's, so the emitter must stay alive and dies unanswered whenever the consuming surface restarts. The emitters most likely to attach an action are precisely the ones that cannot survive that (one-shot commands, installers, automations that restart the surface they just notified). Carrying the effect as data lets the emitter exit at once and lets a record restored after a restart behave exactly like a live one. Three binding rules: a pre-resolved argument vector never a re-parsed string, so foreign-origin values inside it stay arguments and can never become commands (EM-1 + CP-1); referenced resources the emitter may reclaim are copied into the record, since a durable record pointing at a transient original decays; and the accepting surface enumerates its fields and rejects unknown ones, because the field carrying the effect is the only door to execution and a general pass-through is a second one. |

...
- [l1-evidence-archive.md](l1-evidence-archive.md) - [ADDED v1.4.0] The durable substrate CC-3's recoverable original rests on (CC-12), and the owner of the archive-before-reduce ordering (EA-1) that extends this spec's recoverability discipline to the lossy stages CC-1 deliberately excludes.
...
- **CC-12 (Archive-backed recoverability):** [ADDED v1.4.0] CC-3's recoverable original is guaranteed **for the compressed content's lifetime**, which without a durable substrate expires the moment the content leaves the live context — so a recoverability promise that lapses exactly when the session grows long is not one. The durable substrate beneath it is the evidence archive (`l1-evidence-archive` EA-1/EA-3): the pre-compression original is archived before the transform runs, and the compressed form carries the handle that expands back to it (EA-2). Two consequences follow. First, CC-3 no longer decays into "recoverable while it is still in context". Second, the CC-1 boundary — compression is *not* summarization and their guarantees are separate — stops leaving the lossy stages unprotected: CC-10 secures the *durable knowledge* they would destroy, EA-1 secures the *raw content* itself, and the archive-before-reduce ordering applies to every stage of §4.1, not only to this one.
...
| --- | --- | --- | --- |
| 1.4.0 | 2026-08-20 | Core Team | Added CC-12 (archive-backed recoverability) — CC-3's "recoverable original" is scoped to the compressed content's *lifetime*, which without a durable substrate ends exactly when a session grows long enough to need it; the evidence archive (`l1-evidence-archive` EA-1/EA-2/EA-3) is that substrate, so the original is archived before the transform and the compressed form carries an expandable handle. Also closes the second-order gap CC-1's deliberate compression-is-not-summarization boundary leaves open: CC-10 protects the durable *knowledge* the lossy stages would destroy, EA-1 protects the raw *content*, and archive-before-reduce applies across every §4.1 stage rather than only the reversible one. Related Specifications extended with l1-evidence-archive. |

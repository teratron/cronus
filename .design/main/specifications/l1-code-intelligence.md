...
- **CI-2 — Typed entity + relationship taxonomy.** Nodes belong to a closed kind set (at minimum: file, function/method, class/struct, module/namespace, type, interface/trait, variable/constant) and edges to a closed directed-relationship set (at minimum: contains, calls, imports, implements, extends, references; with optional runtime-call edges). Every node and edge carries a flexible property bag (e.g., line span, visibility, signature, call-site line, imported symbols). Edges are directed and may be queried by direction (incoming / outgoing / both). Each edge kind earns its place by a **distinct question a builder or reviewer actually asks** — beyond the structural minimum, `configures` ("what changes this behavior without a code change?") and `validates` ("what tests, drift-checks, or scoring judge this?", the reverse of which powers test-gap analysis in CI-7) are admissible on that test; a vague verb (`influences`, `supports`, `relates-to`) is not, because it answers no such question and cannot be acted on. The vocabulary is enforced **retroactively**: an older graph carrying inadmissible verbs is normalized to the closed set on load, without regeneration.
...
- **CI-14 — Node summaries for navigation.** A node may carry a compact, bounded summary (≈ one sentence) describing its responsibility, so an agent can decide whether a node is relevant *without reading the source*. Summaries are generated deterministically from local signal first (docstring/leading comment, exported symbols, dominant relations, key imports, community/hub context) and may be upgraded by an optional model pass; each records how it was produced and its format version. Summaries are optional, off by default, never the authority (the source is), and are surfaced in node lookup and context assembly under the same token budget as CI-6. A node MAY also inline a **crux** — the handful of lines that carry the logic (the guard, the skip condition, the state change) — and that excerpt is stored as **verbatim source text, not a line range**: line numbers drift whenever unrelated code shifts above them, the lines that matter do not, so a content-anchored excerpt stays correct as the file around it moves. A regenerable node separates a **machine-owned region** (summary, crux, links, sources — regenerated freely on source change) from a **human-authored region** (notes written below it), and regeneration MUST NOT overwrite the human region.
- **CI-15 — Vocabulary-grounded query.** Natural-language queries are expanded against the graph's *own* label vocabulary before traversal, so a wording mismatch between the question and the indexed entities does not collapse recall to noise. The navigable query surface offers at least: breadth traversal (broad neighborhood context), depth traversal (trace a specific path), shortest-path between two entities, and single-node explanation — each bounded by an explicit token budget and citing the source location of any concrete claim. Two further primitives are distinct enough to name: a **file API-surface projection** (every signature in one file, bodies elided — the shape of the file for roughly a tenth of the tokens of reading it), and an **exhaustive pattern search grouped by enclosing symbol and ranked by that symbol's coupling** — the "*every* occurrence of this pattern" need, which a ranked top-N answer (CI-8) structurally cannot serve.
...
- **CI-18 — Derived index is a disposable local cache; only the integration wiring is shared.** The graph is a regenerable **per-developer** artifact, excluded from version control by the build itself (like a dependency cache), never a committed or reviewed file. What *is* committed and shared is the small **integration wiring** — the host-config entries, hooks, statusline, and instruction-file sections that make each developer's agent use *their* locally-built graph. Committing the graph instead trades this for three failures: merge conflicts on a machine-generated file, staleness that reads as reviewed content, and every developer's uncommitted local edits absent from the shared copy. The property that makes "don't commit it" safe is that rebuilding from scratch is cheap and deterministic (CI-4); an index expensive enough that committing it feels necessary is the design smell. Corollary: excluding the graph from version control MUST NOT also hide it from the agent's own search — where the ignore mechanism is shared with the search tool, the build re-admits the graph to search explicitly.
- **CI-19 — Freshness is pulled by the read, measured against working-tree bytes; the drift report never mutates.** A retrieval call brings the index up to date **before** it answers — comparing a cheap fingerprint of the current working tree to the last build's and re-extracting only what moved — so no background daemon or watcher is required and the query that trips a large rebuild is the one that pays for it. Freshness is defined against the **bytes in the working tree**: version control selects which files are visible, but its commit or staging state is not the freshness key, so an answer reflects uncommitted, unstaged, and staged edits alike. A stat-based fingerprint is the fast path; a content-hash fingerprint is available where stat cannot be trusted. Separately, the explicit **drift report** (the CI-gate / human-facing freshness check) is strictly read-only: it reports staleness and never silently repairs it — diagnosis and remediation are distinct acts (the implicit refresh keeps interactive answers live; the report tells a human or a gate that a rebuild is owed).
- **CI-20 — Context delivery mode is a measured tradeoff, not a silent default.** The context CI-6/CI-7 assemble can reach the agent two ways: **eagerly**, as a bundle injected up front, or **on demand**, as retrieval tools the agent calls when it discovers it needs them (a cold, no-index run is the third baseline). They do not have the same effect. Eager injection makes a run faster — fewer round-trips, lower latency — but tends **not** to raise correctness, because the bundle is a guess made before the task's real needs are known. On-demand retrieval gives up much of the speed but **raises** correctness, because the agent pulls exactly the context the emerged need calls for. A subsystem that offers retrieval MUST expose the on-demand surface (it is the correctness lever) and MAY add eager delivery for latency-bound uses; choosing one silently as *the* mode forfeits whichever axis the caller needed. Composes with [l1-cache-stable-context.md](l1-cache-stable-context.md) (an eager bundle lives in the live zone, never mutated into the frozen prefix) and [l1-agent-tool-ergonomics.md](l1-agent-tool-ergonomics.md) (the on-demand surface must be sufficient-to-stop, or the agent falls back to raw search — ATE-1).
...
| Graph is a git-ignored regenerable cache; only the integration wiring is committed and shared (each teammate rebuilds their own) | **[new]** CI-18; the cheap-rebuild property of CI-4 is what makes "don't commit it" safe. The .gitignore/.ignore corollary (hidden from VCS, re-admitted to search) is part of it. |
| Every query refreshes the graph before answering — no daemon; freshness compares working-tree bytes (uncommitted edits included), VCS state is not the key | **[new]** CI-19; read-triggered lazy refresh as an alternative to a background watcher. The explicit drift check is read-only — it never repairs what it reports (diagnose ≠ remediate, cf. `l1-doctor.md`). |
| Push (eager bundle) vs. pull (on-demand tools) vs. cold — measured opposite optima: push is faster, pull is more correct | **[new]** CI-20; a delivery-mode axis over CI-6/CI-7. Composes with `l1-cache-stable-context.md` (live-zone placement) and `l1-agent-tool-ergonomics.md` (ATE-1 sufficiency-to-stop). |
| Crux stored as verbatim source text, not a line range; human "notes" region preserved across regenerations | **[extend]** CI-14; content-anchored excerpt survives unrelated drift; machine-owned vs human-owned regions in one regenerable node. |
| Closed edge-verb vocabulary where each verb answers a real builder/reviewer question (`configures`, `validates`); vague verbs normalized away on load | **[extend]** CI-2; the taxonomy is enforced retroactively, not just at write time. |
| File API-surface projection (signatures only) + exhaustive coupling-ranked pattern search as named primitives distinct from ranked top-N | **[extend]** CI-15. |
| Per-sub-project independent ranking then fusion so a large partition can't swamp a small one | **[extend]** CI-8; monorepo/multi-repo retrieval fairness; scope-labelled hits; narrowable to one partition. |
| Blast radius of a *diff* as a standalone review artifact: a comment plus a linked interactive view; one cached model call to *name* just the impacted areas (a tier between $0-unlabelled and a full deep build) | **[extend]** CI-7; the scoped one-call enrichment and the comment+linked-artifact output shape are the new parts. |
...
These are candidate capabilities for a future nodus analysis surface; this spec records the relevance, and the nodus workspace realizes it in [../../nodus/specifications/l1-nodus-graph.md](../../nodus/specifications/l1-nodus-graph.md) (NG-1…NG-11 — the workflow-as-typed-graph contract, model-free structural analyses, and blast-radius-of-a-diff, built on nodus's own determinism and content identity).
...
| Version | Change |
| --- | --- |
| 1.3.0 | Delta pass against a fourth code-graph engine (a Node/TS context-graph tool). Added CI-18 (derived index is a disposable per-developer local cache; only the integration wiring is committed and shared; git-ignore/search-re-admit corollary), CI-19 (freshness pulled by the read, measured against working-tree bytes not VCS state; the drift report is read-only — diagnose ≠ remediate), CI-20 (context delivery mode — eager push vs. on-demand pull — is a measured tradeoff: push faster, pull more correct). Extended CI-2 (verbs earn their place by a real builder/reviewer question — `configures`/`validates`; vague verbs normalized away retroactively), CI-8 (per-sub-project independent ranking then fusion so a large partition can't swamp a small one), CI-14 (crux as verbatim content-anchored excerpt, not a line range; machine-owned vs preserved human-owned regions), CI-15 (file API-surface projection + exhaustive coupling-ranked pattern search as named primitives). Ideas-to-Adopt rows extended; §6 nodus realization now points at `l1-nodus-graph.md`. |

...
- **ORC-13 (Ending a dispatch's bookkeeping is a declared disposition, and a handoff release never touches the underlying execution):** [ADDED v1.3.0] when the orchestrator ends its own tracking relationship with a dispatched unit, it declares which of two dispositions applies. A **re-delegation** returns the unit to the pool, reassignable, per ORC-4. A **handoff release** ends the orchestrator's bookkeeping and authority over the dispatch *without* signaling, pausing, or tearing down the delegate's running process — the underlying execution continues as an explicitly **unsupervised** entity outside the orchestration's lifecycle, and the release records which disposition occurred and that the process was deliberately left running, never silently. Conflating the two — reassigning work whose process is still live, or reporting "released" in a way indistinguishable from "stopped" — is the failure this invariant closes; ORC-11 governs what happens to a *failed* delegation, this invariant governs what happens to the *live process behind* a dispatch the orchestrator has simply stopped tracking.

- **ORC-14 (Dispatch authority is minted, generation-bound, and revoked once, permanently, at the first terminal transition):** [ADDED v1.3.0] where a dispatched delegate holds a standing right to report progress or results back into orchestration state, that right is a distinct, **minted** authorization bound to the triple of (dispatch identity, delegate instance identity, delegate's current execution generation) — never to the dispatch identity alone. Verification checks all three; a caller presenting a valid dispatch and delegate identity but a **superseded generation** is rejected, the same bound-element-change lapses the grant discipline `l1-consent-binding` CB-3 states for human consent, applied here to inter-agent delegation authority. The authority is revoked **exactly once, first-write-wins, permanently** — at whichever terminal transition the dispatch reaches first: completion, failure, or an ORC-13 handoff release. A stale or delayed signal arriving after revocation MUST NOT reinstate the authority or be treated as evidence the dispatch is still active; this is the guard against a late heartbeat from an already-settled dispatch masking a genuinely hung retry behind it.
...
| --- | --- | --- | --- |
| 1.3.0 | 2026-09-01 | Core Team | Added ORC-13 (ending a dispatch's bookkeeping is a declared disposition — a re-delegation returns the unit to the pool, a **handoff release** ends orchestration tracking without signaling or tearing down the delegate's still-running process, which continues as an explicitly unsupervised entity) and ORC-14 (dispatch authority is a minted, generation-bound authorization over the (dispatch, delegate, execution-generation) triple, verified on all three so a superseded generation is rejected even with a valid dispatch+delegate identity, composing CB-3's grant-lapses-on-bound-element-change rule for inter-agent delegation; revoked exactly once, first-write-wins, at whichever terminal transition — completion, failure, or ORC-13 handoff — the dispatch reaches first, and never reinstated by a stale or delayed signal). Distilled from an adoption pass over an external desktop multi-agent orchestrator's dispatch-lifecycle and capability-token mechanics. |

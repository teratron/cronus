...
- **ERR-8 (A refusal reports the reasons that caused it, never a metric that correlates with them):** `[ADDED v1.3.0]` when an operation is declined, blocked, or gated, what is surfaced is the **actual set of conditions that produced the decision** — each named — and not a count, score, or summary figure that merely tends to move with them. A figure computed over a *subset* of the deciding conditions is the sharpest form of this failure: it is individually truthful and jointly misleading, so a refusal caused entirely by conditions outside the subset is presented alongside a perfectly accurate "zero", and the reader concludes the system is malfunctioning rather than that something they cannot see is blocking them. Where a count is genuinely useful, it accompanies the named blockers; it never stands in for them. This binds every decline surface — permission gates, capability refusals, resource and lifecycle blocks — not only crash-shaped errors.

- **ERR-9 (A diagnostic reaches the surface that triggered the act, or the act degrades into one that can explain itself):** [ADDED v1.4.0] an error is only reported where it is **deliverable on the surface the action was invoked from**. An action triggered from a surface with **no diagnostic channel** — a keystroke binding, a menu entry, an automation fire, a click on a notification — MUST NOT fail into a channel that surface cannot show (a stream nobody is reading, a log nobody opens): from the person's side that is not an error, it is an act that did nothing, and an act that opens nothing explains nothing. Where a diagnosis cannot be delivered on the triggering surface, the correct behaviour is to **degrade the action into one that can explain itself** — most often the interactive affordance that would have resolved the missing precondition (offer the chooser rather than report the missing default; open the setup rather than report the unconfigured state) — or, failing that, to raise the diagnosis on a **surface of record the person can find afterwards** and to say so. The same rule closes the mirror case on rendering: a surface that **omits a section when its data is missing** has reported nothing, so an unavailable region carries a short statement of why it is unavailable instead of vanishing, and a **degraded** rendering states its degradation rather than presenting a partial answer as a whole one.
...
| --- | --- | --- | --- |
| 1.4.0 | 2026-09-04 | Core Team | Added ERR-9 — a diagnostic reaches the surface that triggered the act, or the act degrades into one that can explain itself. ERR-1…ERR-8 govern the *content* and *consent* of a report and assume a channel exists to carry it; the assumption fails for every action invoked from a surface with no diagnostic channel at all — a keybinding, a menu row, an automation fire — where writing the error to a stream nobody reads produces, from the person's side, an act that silently did nothing. The sanctioned degradation is into the interactive affordance that resolves the missing precondition (offer the chooser instead of reporting the missing default), else a findable surface of record with that fact stated. Extended to the rendering mirror: a section that disappears when its data is absent has reported nothing, so an unavailable region states why, and a degraded rendering declares its degradation rather than presenting a partial answer as whole. |
| 1.3.0 | 2026-08-20 | Core Team | Amendment — ERR-8: a refusal reports the conditions that actually caused it, each named, never a count or score that merely correlates with them. The sharpest form of the failure is a figure computed over a *subset* of the deciding conditions: individually truthful and jointly misleading, so a refusal caused entirely by conditions outside the subset arrives beside an accurate "zero" and the reader concludes the system is malfunctioning rather than that an unseen condition is blocking. A count may accompany named blockers but never substitutes for them. Binds every decline surface — permission gates, capability refusals, resource and lifecycle blocks — not only crash-shaped errors. Additive; L1 stays Stable (C9). Distilled from an adoption pass over an external agent-orchestration client whose restart-refusal dialog quoted a truthful session count that measured none of the conditions actually denying the request. |

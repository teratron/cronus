...
- **CE-11 (Variants are differentiated by assigned constraint; a design contest settles by deliberation, not by this contract):** [ADDED v1.2.0] two things change when a contest explores a **design space** rather than executing one already-settled design. First, **N independent samples of the same brief converge**: the attempts differ in wording and agree in substance, and CE-7's budget buys near-duplicates. Such a contest therefore **assigns each attempt a different, declared constraint** — minimize the interface, maximize extensibility, optimize the most common caller, design around the seam — so the variants are *made* to differ rather than hoped to, and comparison runs on **axes declared before the attempts start** rather than on an impression formed after reading them. Second, and this is a **boundary, not a permission**: a design exploration's output is a **decision**, not an artifact to integrate, so its settlement is **not this contract's**. CE-4 and CE-5 hold unchanged wherever a tournament produces an artifact — one whole attempt as authored, losers discarded, no composite assembled. Where the goal is to choose a *design*, the tournament supplies the candidates and the **deliberation** protocol settles them, which is where a reasoned blend is legitimate and is recorded as an authored position with named provenance. Routing a design exploration through CE's selection rule yields the worst of both: a whole artifact crowned for one design idea, with the rest of the space discarded unexamined.
...
| 1.2.0 | 2026-08-26 | Core Team | Amended — CE-11: **design-space** contests. N independent samples of one brief converge, so CE-7's budget buys near-duplicates; such a contest **assigns each attempt a different declared constraint** (minimize the interface, maximize extensibility, optimize the common caller, design around the seam) so variants are *made* to differ, and compares on **axes declared before the attempts start** rather than on an impression formed afterwards. The second half is a **boundary rather than an exception**: a design exploration's output is a *decision*, not an artifact to integrate, so CE-4/CE-5 stand unchanged for artifact tournaments (one whole attempt as authored, losers discarded, no composite) and a design contest hands its candidates to the **deliberation** protocol, where a reasoned blend is legitimate and recorded with named provenance. Routing a design exploration through CE's selection rule crowns a whole artifact for one design idea and discards the rest of the space unexamined. Distilled from an adoption pass over an external engineering-skills reference. |

...
- [l1-launch-handoff.md](l1-launch-handoff.md) — LH-5 is SP-11 read from the entry point: the launcher may not resolve a catalog name, because the catalog does not exist until the composition that owns it has mounted.
...
- **SP-11 (Shared vocabulary, declared exposure — omission never means absence):** `[MODIFIED v1.1.0]` the actions a product offers are described **once**, in a catalog carrying each action's identity, human-readable name, and **resolution locus**. The locus set is closed and has four members: **semantic** (belongs to the core; reachable from every surface and remotely invocable), **client-local** (belongs to a surface; the same identity may have a different implementation per surface), **host-only** (never remotely invocable — a host-owned facility, declared with its reason), and **installation** (acts on the product's own installation rather than on the user's work — it runs to completion without a session and never appears on a session-scoped surface). Menus, help, palettes, bindings, and agent-facing instructions all render from that catalog. An action a given surface deliberately does not expose is **declared** as such in the catalog: a capability boundary stated as an exclusion is a scope decision, while the same boundary expressed by silence is indistinguishable from a missing feature and will be "fixed" by someone.

- **SP-12 (A catalog entry has a projectable face and an executable one, and the split is declared per member):** `[ADDED v1.1.0]` every entry divides into what may be **projected** — identity, name, summary, argument schema, locus, exposure — and what may not: the behavior itself, the host handles it closed over, the state it captured. The projectable face crosses every boundary the catalog crosses, including a process boundary and a wire. The executable face **never leaves the host**, and a member that has not been declared projectable does not cross. The declaration is per member and explicit, because both defaults fail: an entry that is wholly serializable ships handles to a remote surface that cannot honor them, and an entry that is wholly opaque forces every remote surface to restate the catalog — which is the fork SP-11 exists to prevent, reintroduced at the process boundary where it is hardest to see.

- **SP-13 (Unrecognized and unavailable are different answers; a surface needs all three):** `[ADDED v1.1.0]` resolving an input against the catalog yields exactly three outcomes, and a surface must be able to tell them apart: the input **named an entry**, the input **named no entry at all**, or it named one that **could not answer**. The middle outcome is not a failure and must not be reported as one. It is what lets a surface fall through — a line that is not a command is ordinary input, a token that is not a verb may belong to a different parser — and folding it into the third denies that fall-through, while folding it into the first denies the user the plain *no such action* they need. Two of the three are cheap to conflate and each conflation removes a behavior nothing else can supply.
...
| 1.1.0 | 2026-09-06 | Core Team | Amendment reconciling the catalog with observed world practice. SP-11 gains a fourth **installation** locus: verbs that act on the product's own installation run to completion without a session and belong on no session-scoped surface, a class the semantic/client-local/host-only triad had no member for and which was therefore being modelled as semantic. SP-12 splits a catalog entry into a **projectable face and an executable one with a per-member declaration**, because both defaults fail — a wholly serializable entry ships host handles across a wire, a wholly opaque one forces every remote surface to restate the catalog. SP-13 separates **unrecognized from unavailable**: resolution has three outcomes, and collapsing the middle one removes a surface's ability to fall through on input that was never a command. |

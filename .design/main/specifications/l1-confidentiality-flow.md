...
- **CF-11 (An opt-in egress channel is a machine-enforced allowlist, not a review rule):** where the system is ever configured with an outbound channel that is *not* the user's own data path — anonymous telemetry, a diagnostics ping, a crash report — that channel is constrained by construction, and the constraint is the invariant, not a code-review convention:
  - **Field-level allowlist, fail-closed at the carrier.** The set of event names and, per event, the set of property keys that may cross is a closed declaration. The send primitive **drops** an unlisted event or key rather than trusting its caller — so a future call site that carelessly adds `path` to an event sends the event *without* `path`, and the blast radius of the mistake is "a metric is lost", never "a source tree is received".
  - **Values are buckets and closed enums, never free-text or identifiers.** A number crosses as a range bucket (`100–500`, not `437` — a raw count plus a language set fingerprints a specific repository); a string crosses only as a member of a fixed union. A raw error message is a path, a symbol, and sometimes a snippet of code; an enum member is not. No field carries user content, a file path, a repository or symbol name, a query, or a raw error string.
  - **The channel has a dry-run.** A command prints the **exact payload** the machine would send, so the claim is verifiable by inspection (reinforces CF-9) — the auditable artifact is a single pure declaration with no I/O, config, or clock, that *is* the enforced contract.
  - **Default-off in the contexts where consent is absent or unclear** — CI, and any build from source — independent of the runtime opt-out, which remains available by at least one well-known mechanism.

  This composes CF-4 (a sink with a declared capacity) and CF-8 (enforced by mechanism at the sink, never by instructing a caller); CF-11 fixes the *shape* that capacity takes for a diagnostics channel.
...
| --- | --- | --- | --- |
| 1.1.0 | 2026-09-09 | Core Team | Added CF-11 (an opt-in egress channel is a machine-enforced allowlist, not a review rule) — mined from a fourth code-graph engine's telemetry contract: a closed event→key allowlist the send primitive fail-closes on (an unlisted key is dropped, not sent), values as range buckets and closed enums never free-text or identifiers (a raw count plus a language set fingerprints a repo), a dry-run command that prints the exact payload (reinforces CF-9), and default-off in CI and source builds. Composes CF-4 (sink capacity) and CF-8 (mechanism at the sink); it fixes the shape that capacity takes for a diagnostics channel. Additive; no existing invariant weakened. |

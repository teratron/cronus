...
- [l1-staged-rollout.md](l1-staged-rollout.md) - [ADDED v1.2.0] Owner of the observe → warn → enforce ratchet (SR-10) that PG-10 binds the managed tier to when a restriction is newly introduced or tightened.
...
- **PG-10 (A newly introduced restriction ratchets; it never binds on first contact):** [ADDED v1.2.0] when a managed restriction is **newly introduced or tightened** — a lockdown of a loadable surface (PG-4), a narrowed trusted-source set (PG-5), a lowered cap — it MUST NOT go from absent to blocking in one step. It is first applied in **observe** mode, in which it blocks nothing and records exactly what it *would have* blocked; then in **warn**, in which the affected operation still succeeds while the principal is told the restriction now covers it; and only then in **enforce**. The reason is specific to this tier: a managed restriction is set out-of-band by an administrator (PG-2), applies to everyone at once, and cannot be overridden by the people it breaks (PG-1) — so a mis-sized restriction has the largest blast radius and the fewest escape routes of any change in the system, and the measurement that would have sized it correctly is available for free before it binds. The ratchet governs *introduction only*: an already-enforcing restriction stays enforcing, and PG-7's fail-safe-presence posture is unchanged. Demoting `enforce → warn` remains available as the immediate remedy for a restriction discovered to be mis-sized, without editing any threshold. The full ratchet semantics, including the derived warn threshold, are owned by `l1-staged-rollout` (SR-10); PG-10 binds the managed tier to them.

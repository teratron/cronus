...
- **FC-13 Frontier-batched elicitation, recommended answers, and the fact/decision split**: [ADDED v1.2.0] where facilitation is **eliciting content** rather than offering process choices, the open questions form a **dependency tree**: a question whose answer depends on another question still open belongs to a **later round**, never this one. The agent asks the whole **frontier** — every question whose prerequisites are already settled — in one round, each carrying the agent's **recommended answer** so a satisfied user can accept in a word, then waits. Each round's answers push the frontier outward; the session completes when the **frontier is empty**, never when a count is reached. Two rules bound it, and they are exactly what distinguishes this from the stacked-question wall FC-4 forbids. **Facts are the agent's job; decisions are the user's** — anything discoverable from the environment is looked up rather than asked, dispatched in parallel, and blocks only the questions downstream of it. And FC-4's prohibition governs the **process** menu, where no recommendation can stand in for the user's pick; a content question arriving with a recommended answer attached is a proposal, which is the opposite of a wall. A **hard cap on question count is rejected**: it conflates a genuinely under-specified subject (the mechanism working) with low-value questioning (a quality defect whose fix is the prompt, never a counter).

- **FC-14 Stake tiering inside the frontier, and the consequence line is the admission test**: [ADDED v1.3.0] FC-13 decides *which* questions may be asked now (dependency); this invariant decides *how* they are asked (consequence). Within one frontier round, questions are tiered by **what a wrong answer costs**. A **load-bearing** question — one whose wrong answer costs a migration, a rewrite, a security hole, or lost trust — is asked **singly and in sequence**, because each answer reshapes the questions after it and a batch would freeze that reshaping out. Everything else is issued as **one block of recommendations the user vetoes by exception**; silence accepts it. Every question, in either tier, carries three parts: **why it is load-bearing**, the agent's **recommended answer**, and the **named consequence of guessing wrong**. The third part is not decoration — it is the **admission test**: a question whose consequence line comes out weak *is not load-bearing*, and is demoted into the veto-by-exception block rather than asked. This is the mechanical fix for the defect FC-13 names and leaves unnamed ("low-value questioning, a quality defect whose fix is the prompt, never a counter"): the counter FC-13 rejects is replaced by a per-question test the author must pass. A question that survives the test but whose answer is discoverable is still the agent's job, not the user's (FC-13).

- **FC-15 Bulk acceptance is a granted delegation over a named set, never an inference from silence**: [ADDED v1.3.0] the user MAY, in **one act**, accept every remaining recommendation and end the elicitation. Three conditions make this a legitimate human decision rather than the agent answering on the human's behalf. It is **offered, never assumed** — the agent surfaces it as available (and does so explicitly once the load-bearing tier grows past the point where one-at-a-time is serving anybody), and it takes an affirmative act. It **names the set it covers at the moment it is given** — the specific open questions and the specific recommendations that will be locked, in their resolved form, so the grant attaches to what will actually happen rather than to an intention (the resolved-invocation discipline of consent, CB-1/CB-2). And every decision it locks is **marked in the artifact as accepted-by-default**, so a later reader can distinguish a deliberated choice from a defaulted one and knows which to re-open first. Absent all three, an "accept everything" is a fabricated counterparty (XPL-4) wearing a shortcut's clothes: silence is not this grant, an unnamed set is not this grant, and an unmarked decision destroys the only evidence that it was one.

- **FC-16 The resolved set is presented once, sourced, and a correction re-opens the question**: [ADDED v1.3.0] the facts FC-13 makes the agent's job are presented to the user as **one batch, once, before questioning begins** — never dripped as individual questions and never as bare assertions. Each entry **names the source that settled it**: the artifact, the location, the convention, or the research finding. Sourcing is what makes the batch reviewable at a glance rather than a wall of claims to be taken on faith, and it is what lets a user correct one entry cheaply. A **correction to any entry is not a note — it re-opens that entry as a question** in the current frontier, along with anything downstream that rested on it, because a settled fact and a wrong fact differ in exactly one way that matters: what the rest of the session was allowed to assume. An entry left uncorrected stands as confirmed; the batch is presented *as* that contract, not silently treated as one.
...
| 1.2.0 | 2026-08-26 | Core Team | Amended — FC-13 frontier-batched elicitation: open questions form a **dependency tree**, a question depending on another still-open question belongs to a later round, and the agent asks the whole **frontier** in one round with a **recommended answer** attached to each, completing when the frontier empties rather than at a count. **Facts are the agent's job, decisions the user's** — environment-discoverable facts are looked up in parallel and block only their downstream questions. Resolves the apparent tension with FC-4 explicitly: FC-4 governs the *process* menu (where a recommendation cannot substitute for the user's pick), while a content question carrying a recommendation is a proposal, not a wall. A hard question cap is rejected as conflating an under-specified subject with low-value questioning, whose fix is the prompt rather than a counter. Distilled from an adoption pass over an external engineering-skills reference. |
| 1.3.0 | 2026-09-01 | Core Team | Amended — FC-14 stake tiering inside the frontier: FC-13 decides which questions may be asked now, FC-14 decides how, tiering them by what a wrong answer costs — load-bearing asked singly and in sequence because each answer reshapes the rest, everything else issued as one veto-by-exception block; every question carries why-it-is-load-bearing, a committed recommendation, and a named consequence-if-wrong, the last being the **admission test** (a weak consequence line demotes the question), which is the mechanical fix for the low-value-questioning defect FC-13 names and leaves unnamed. FC-15 bulk acceptance as a granted delegation: offered never assumed, naming the resolved set it covers at the moment it is given, and marking every decision it locks as accepted-by-default — absent all three it is a fabricated counterparty (XPL-4) in a shortcut's clothes. FC-16 the resolved set presented once as a sourced batch before questioning begins, each entry naming what settled it, and a correction re-opening that entry as a question along with everything downstream of it. Mined from an external cross-model plan-hardening skill collection. |

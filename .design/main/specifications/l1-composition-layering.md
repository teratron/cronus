...
- **LAY-11 (A layer that fails to load is a failed layer, never an empty one):** [ADDED v1.1.0] where a layer cannot be read, parsed, or validated, the composition records it as **failed and reports it**, and MUST NOT treat it as a layer that contributed nothing. The two are indistinguishable in the composed result and completely different in meaning: an empty layer is a statement the author made, a failed layer is a statement that never arrived. This is exactly why the failure is silent in practice — the stack below still composes, the effective set is still valid, every surface renders normally, and the only observable symptom is that everything the broken layer declared is missing, which reads as *the layer was never written*. LAY-4 reports an override matching nothing and LAY-5 distinguishes an empty declaration from an absent one; this closes the same family's third member at the **layer** grain rather than the entry grain. The disposition is declared, not inferred: a failed layer either **blocks recomposition** (LAY-10's transactional rejection, keeping the last good set in force) or is **excluded with a standing, visible defect** — never dropped quietly, and never silently re-attempted on every load in a way that normalizes the drop.
...
| --- | --- | --- | --- |
| 1.1.0 | 2026-09-04 | Core Team | Added LAY-11 — a layer that fails to load is a failed layer, never an empty one. LAY-4 (an override matching nothing is reported) and LAY-5 (empty ≠ absent) close the entry grain; the **layer** grain was open, and it is where the failure is invisible: an unparseable user layer contributes no entries, the stack below still composes, every surface renders a valid effective set, and the only symptom is that everything the author declared is missing — which reads as never having written it. The disposition is now declared rather than inferred: block recomposition per LAY-10, or exclude the layer with a standing visible defect; never a quiet drop, and never a silent re-attempt on every load that normalizes the drop. |

...
- [l1-output-contracts.md](l1-output-contracts.md) - [ADDED v1.1.0] the bounded validate-retry-with-feedback loop (OC-4/OC-5/OC-6) that CM-10 composes to repair a self-mis-anchored delta before it is surfaced as a terminal stale-base blocker (CM-9).
...
- **CM-10 Self-mis-anchoring is repaired, not merged** [ADDED v1.1.0]: a stale-base divergence (CM-4) has **two distinct causes** that demand different remedies, and conflating them is the defect this invariant closes. **Concurrent divergence** — the live unit moved because *another* in-flight change touched it — is reconciled by the three-way merge (CM-5), because there is a genuine competing edit to integrate. **Self-mis-anchoring** — the producer's delta simply does not fit the *current* artifact because it was authored against a **stale or mis-remembered base** with **no** competing concurrent change — has nothing to three-way-merge: the delta is not in conflict with another edit, it is wrong about what the unit contains. For this case the remedy is a **bounded, automated re-anchoring repair**: the **producing actor** is handed the **precise mismatch** — which unit, the base it recorded, and the actual current content — and re-emits *just the failed delta* re-anchored against the truth, under a declared retry budget (composing the output-contract retry-with-verdict-injection loop, OC-4/OC-5). The terminal stale-base blocker (CM-9) is **deferred until the repair budget is exhausted**, not raised on first failure. Three rules bind it. (a) **Precise diagnostic, targeted repair — not regenerate-from-scratch**: the repair is given the exact anchor mismatch and re-anchors the existing intent, because the intent was sound and only its anchor was stale; discarding the whole edit to re-reason from zero is wasteful and loses the producer's work. (b) **Repair re-anchors the same intent; it is not a licence to change what the edit means**: the producer re-fits its own edit to the current content, and the runtime never silently replaces the diverged unit with the stale delta (the CM-4 prohibition holds throughout). A repair that, under cover of re-anchoring, **widens the change** — touching units the original delta did not, or altering the semantic effect — is a **new change proposal** subject to the same review and gates as any other, not a free continuation of the failed one; the repair budget buys re-anchoring, never scope-creep. (c) **Exhaustion surfaces the precise failure, atomically**: when repair exhausts, the change is surfaced as a terminal blocker naming *which units and which hunks* could not be anchored (CM-9), and the change-set is applied wholly or not at all (CM-8) — a partially-anchored edit set is never committed. The *encoding* of a delta bears on how often self-mis-anchoring occurs: a whole-unit replacement has no anchor to miss, a targeted diff has the most, and a search-anchored form sits between — an implementation MAY offer interchangeable encodings trading verbosity for apply-robustness, but the CM-10 repair contract is independent of which encoding produced the delta.

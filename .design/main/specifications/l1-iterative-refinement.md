...
- [l1-review-checkpoint.md](l1-review-checkpoint.md) — [ADDED v1.2.0] the human-grader case of this loop (IR-10): a solicited review whose request-revision is the continue-verdict and whose free-form feedback is the actionable signal; the checkpoint's pause/decision-set/routing are owned there, the revise loop here.
...
- **IR-10 (The grader MAY be a human, and a request-revision is the continue-verdict):** [ADDED v1.2.0] the independent grader of IR-2 MAY be a **human reviewer** rather than an automated judge, in which case a **request-revision** decision is the loop's *continue* verdict and an **approve** is its *pass*: the reviewer's **free-form feedback** is the actionable signal IR-3 threads into the producer's next attempt, in place of a rubric line. The human-grader case does not relax the loop's discipline — it stays **bounded** with an honest exit (IR-5), its criteria stay **frozen** for the loop's lifetime (IR-6, a reviewer may not move the goalposts mid-loop any more than an automated one), the producer stays **independent of its own judge** (IR-2, a producer cannot review or approve its own work), and every cycle stays **recorded** (IR-7). What the human case adds — the solicited pause, the closed approve/revise/reject decision set, the return-to-producer routing, and the default-review-on-the-terminal-step bias — is owned by the new `l1-review-checkpoint` contract (RC-1…RC-10); IR-10 records only that a human review *is* an instance of grader-gated refinement, so the two compose rather than duplicating a revise loop.
...
| --- | --- | --- | --- |
| 1.2.0 | 2026-07-23 | Core Team | Added IR-10 — the independent grader (IR-2) MAY be a **human reviewer**, in which case a **request-revision** is the loop's continue-verdict and an **approve** its pass, with the reviewer's **free-form feedback** the actionable signal IR-3 threads into the next attempt in place of a rubric line. The human-grader case relaxes none of the loop's discipline — bounded honest exit (IR-5), frozen criteria so a reviewer cannot move goalposts mid-loop (IR-6), producer independent of its own judge (IR-2), every cycle recorded (IR-7). What the human case *adds* — the solicited pause, the closed approve/revise/reject decision set, the return-to-producer routing, and the default-review-on-the-terminal-step bias — is owned by the new `l1-review-checkpoint` (RC-1…RC-10); IR-10 records only that a human review is an instance of grader-gated refinement so the two compose rather than duplicating a revise loop. Related Specifications extended with `l1-review-checkpoint`. |

...
- **DC-11 (Layered resolution with per-field authority — a lower-trust layer may tune, never govern):** `[ADDED v1.1.0]` an effective value is resolved from an **ordered stack of layers** (built-in default → the operator's own settings → a project's or workspace's settings → an explicit per-invocation choice), and each **field declares the lowest-trust layer permitted to set it**. Three rules make the stack safe rather than a hole in the authority model:

  1. **Environment-owned fields are operator-only.** A field that describes the *operator's environment* rather than the *component's behavior* — input bindings and shortcuts, display preferences, paths to local tooling, credentials — MUST NOT be settable by a layer the operator did not author. What a key does is a property of the operator's habits, not of a project they happened to open; a checkout that can rearrange the operator's controls has been handed a capability nobody granted it.
  2. **A value from a lower-trust layer never feeds an exec-adjacent decision.** Values that resolve to an executable path, a command line, a module to load, or a network destination are taken only from a layer the operator controls. Configuration merges by field identity with no memory of where a component was installed from, so an untrusted layer can otherwise reach a globally-installed component's settings — which is fine for tuning and unacceptable for anything that decides what code runs.
  3. **Lower-trust configuration is disclosed, not silent.** Where a lower-trust layer sets values for a component, the fact is **surfaced** — which components a project configures, at minimum — because the surprising case is not a bad value but an unnoticed one. Project-level tuning of a shared component is a normal, wanted workflow; it just may never be invisible.

  A field's declared authority floor is part of its declaration (DC-1) and is validated by the owner like any other constraint (DC-3); an attempt by a layer below the floor is **refused with the reason**, never silently dropped and never silently applied.

- **DC-12 (Silence is a lenient default; an explicit declaration is a strict contract — and it may not be withdrawn to pass):** [ADDED v1.2.0] an optional field that is **omitted** leaves the owner free to choose a safe, best-effort behaviour and to degrade it quietly when circumstances do not allow it. The same field **declared** is a statement of intent, and the owner MUST thereafter satisfy it or **reject the configuration with a diagnostic** — never silently fall back to the omitted-case behaviour, which converts an author's explicit request into an invisible no-op. Two consequences bind. Adding a declaration **raises the bar rather than lowering it**, so an author is never worse off for having been explicit — the failure this prevents is the one where declaring a preference makes the surface *more* permissive because the declared path has weaker checks than the default path. And a declaration may **not be removed in order to pass validation**: withdrawing it changes what the configuration claims, so the repair is to satisfy the declaration or to report the diagnostics truthfully, and dropping it to reach a green result is DC-3's rejection being routed around rather than answered (composing AG-8 — governable stricter, never silently laxer).
...
| --- | --- | --- | --- |
| 1.2.0 | 2026-09-04 | Core Team | Added DC-12 — silence is a lenient default, an explicit declaration is a strict contract, and it may not be withdrawn to pass. An omitted optional field leaves the owner free to pick a safe best-effort behaviour and degrade it quietly; the same field **declared** is a statement of intent the owner must satisfy or reject with a diagnostic, never silently serve with the omitted-case behaviour — which turns an author's explicit request into an invisible no-op. Two consequences: declaring **raises** the bar, so an author is never worse off for being explicit (the prevented failure is a declared path with weaker checks than the default path); and a declaration may not be dropped to reach a green result, since withdrawing it changes what the configuration claims and is DC-3's rejection routed around rather than answered (AG-8). From an external artifact-generation tool where an omitted presentation field yields a truthful auto-derived default while declaring it makes the presentation intentional and strict, and where a declared engineering profile explicitly may not be removed merely to pass validation. |

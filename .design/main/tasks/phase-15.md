...
- [ ] [T-15A01] Capture-metadata schema: `actor`/`expiry`/`subject`/`cross_ref` fields on `MemoryEntry` + columns, absent-by-default (MI-6 net-new fields)
...
- **Status:** Todo
- **Assignment:** Agent
- **Verify:** `cargo test -p cronus-store-local` + `-p cronus-contract`: `MemoryEntry` gains `actor: Option<String>` (who-said-it, distinct from scope ownership), `expiry: Option<u64>` (a hard void-after instant, complementing MEM-5 decay — not a decay knob), `subject` (a closed enum: about-user vs about-agent-self), and `cross_ref: Vec<MemoryId>` (related item ids named at capture). Each maps to a nullable/empty-default column so the **entire pre-existing corpus reads back at baseline with no migration step** (the additive-column pattern proven 4× in Phase 14 — assert an old-shape row round-trips with all four absent). A round-trip write/read test proves each field persists and `map_row` decodes it; a builder (`with_actor`/`with_expiry`/`with_subject`/`with_cross_ref`, mirroring the existing `with_workspace`/`with_depth`) sets them without disturbing existing call sites.
- **Handoff:** T-15B01 attributes/sets these fields through the capture policy; T-15B03 records directive provenance alongside them.

...
- [ ] [T-3A01] `crates/cli` binary scaffold — Cargo.toml + main.rs + clap command skeleton + exit codes + output mode

Track B — Core commands

- [ ] [T-3B01] `cronus help` + `cronus status` — usage discovery + current workspace/phase display
- [ ] [T-3B02] `cronus init [path]` — workspace initialization via core state bootstrap

Track C — Workflow command group

- [ ] [T-3C01] `cronus workflow scaffold <name>` — generates `.nodus` file skeleton
- [ ] [T-3C02] `cronus workflow validate <file>` — validates + renders diagnostics; exits non-zero on errors
- [ ] [T-3C03] `cronus workflow run <file> [--input '{}']` — executes workflow + renders RunResult
- [ ] [T-3C04] `cronus workflow transpile <file> [--human|--compact]` — renders transpiled form to stdout

Track T — Validation & parity

- [ ] [T-3T01] CLI integration tests + command exit-code contract

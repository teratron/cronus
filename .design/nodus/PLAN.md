...
- [ ] **L2 Nodus Restart** ([l2-nodus-restart.md](specifications/l2-nodus-restart.md)) [L2] — Track A: `restart_max` in `§runtime:` + `1..=10` bound check (mirroring `e017_retry_bounded`) + transpiler round-trip; `RESTART_LIMIT` (Warn) / `RESTART_SCOPE` (Error) codes; `$restart` reserved-but-writable + `$restart_count` reserved-and-runtime-owned (NL-8 — the count cannot be forged by the flow logic that reads it). Track B: `Signal::Restart` (private enum, no public-API or `Value` change) raised only from a top-level step + the static nested-request rule; the bounded attempt loop + `RESTART_LIMIT` on exhaustion. Track T: bound + authority + freshness + `§config` re-read + additivity baseline + zero-dep

## Phase 19 — Compensation Seam (l2-nodus-compensation)

*Realize `l2-nodus-compensation.md` (Stable v1.0.0) in `crates/nodus`, per NL-22: an effectful step may declare a host-supplied compensating action, and on scope failure/cancel/explicit-request the compensations of its **successfully-completed** steps run in reverse order of completion (LIFO — a correctness contract, since later effects were built on earlier ones, so the ledger is popped rather than re-read forward). Two records, not one: the completed-effect ledger **and** a parallel un-compensable record, so a committed effect with no declared undo is explicitly recorded rather than inferred from absence. Three compositions NL-22 names are vacuous in core today and must not be invented — the LP-11 gate (compensations route through the one existing `execute_command` path, inheriting the seam for free when it lands), NL-12 crash-resume (at-least-once per process run, ledger as the host's replay artifact), and a declared sub-region scope (the run is the scope this phase). Purely additive. Sequential. Atomic tasks in [tasks/phase-19.md](tasks/phase-19.md).*

- [ ] **L2 Nodus Compensation** ([l2-nodus-compensation.md](specifications/l2-nodus-compensation.md)) [L2] — Track A: `TildeCompensate` token (matched before the generic `~identifier` flag rule, or it mis-lexes as a `Flag`) + `Step.compensation: Option<CommandCall>` mirroring the `Step.retry` precedent + `~COMPENSATE: CMD(args)` trailing same-line clause + transpiler round-trip; `NODUS:COMPENSATION_FAILED` code. Track B: ledger + un-compensable record populated on step completion; arming condition (`Failed`/`Aborted`/explicit only) + LIFO drain + failure recording that leaves the original effect live and **continues** the unwind. Track T: order + completed-only + fallible + armed + honesty + `!!` bypass + observer neutrality + zero-dep
...
<!-- Pending L2 Invariant-Compliance obligations (additive invariants on already-Done L1 concept specs; catalogued at INDEX v1.0.57, entries marked RESOLVED as they land — no running version stamp to re-drift) — awaiting L2 realization specs before they can be decomposed into verifiable atomic tasks:
  ...
  - NL-22 (compensation seam): RESOLVED — l2-nodus-compensation authored (Stable v1.0.0) and phased as Phase 19. The design fork over its three absent seams (LP-11 gate, LP-17 settlement, NL-12 resume-for-effects) was settled at spec time by recording each as vacuous-in-core-today, the Phase-15 streaming precedent.
  - NL-23 (bounded whole-run self-restart): RESOLVED — l2-nodus-restart authored (Stable v1.0.0) and phased as Phase 18.
  ...
  - `l2-nodus-restart` §5/INDEX wording inaccuracy (found at Phase-18 plan time): §5 slice 3 and the `INDEX.md` row loosely group `$restart` into `RUNTIME_OWNED_VARIABLES` alongside `$restart_count`. Only `$restart_count` belongs there — §3's Invariant Compliance row is correct, and a runtime-owned `$restart` would make the request an `E013` error to assign, rendering the feature unusable. Phase 18 plans to §3 and carries the discrepancy as an explicit guardrail; the wording needs a patch-level correction on the next `/magic.spec` touch of that file. -->

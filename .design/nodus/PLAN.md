...
**Based on:** .design/nodus/INDEX.md v1.0.65
**Status:** Active
...
<!-- Pending L2 Invariant-Compliance obligations (additive invariants on already-Done L1 concept specs; catalogued at INDEX v1.0.57, entries marked RESOLVED as they land — no running version stamp to re-drift) — awaiting L2 realization specs before they can be decomposed into verifiable atomic tasks:
  ...
  - `l2-nodus-compensation` §4.4/canonical-example inaccuracies (found during Phase 19): the spec's illustrative example uses `UNPUBLISH`, which is not a real nodus command (absent from `KNOWN_COMMANDS`) — cosmetic, but worth a patch so a reader can't copy it expecting it to parse as a command. More substantively, §4.4's ledger design is specified with a parallel `uncompensable` record and an implied per-entry outcome, neither of which was built (nothing reads either — see Phase 19's STATE.md decision). Needs a `/magic.spec` patch reconciling §4.4 with the simpler as-built ledger (`CompletedEffect { step_number, compensation }` only), matching how Phase 18's `l2-nodus-restart` deviations were recorded.
  ...
  - `l2-nodus-restart` §5/INDEX wording inaccuracy: RESOLVED at implementation time (Phase 18 planned to §3, the authoritative row); still needs a patch-level wording correction to §5/INDEX on the next `/magic.spec` touch of that file so the two sections stop disagreeing.
  - `l2-nodus-restart` §4.2/§4.6 implementation deviations (found during Phase 18): the spec calls for a `Signal::Restart` variant plus a `vocab.rs` `NODUS:RESTART_SCOPE` constant. Neither was built — grounding showed every comparable structural rule in this crate (E010/E013/E014/E016/E017) is validator-only, with zero runtime re-checks, because `Executor::execute` can be called directly with an unvalidated AST (a real, precedented pattern in this crate's own tests) and `Signal`-based interception would have been the first redundant enforcement layer. `RESTART_SCOPE` landed as a bare validator code (`E019`) instead. Needs a `/magic.spec` patch to `l2-nodus-restart.md` reconciling §4.2/§4.6 with what was actually built, and to record why (the validator-only precedent).
  - Pre-existing parser defect (found during Phase 18, unrelated to NL-23): inline `?IF cond → CMD(args) → $target` never attaches the trailing pipeline target — `try_parse_command_from_string` parses `NAME(args)` and discards everything after the closing paren. Affects any workflow using that syntax, not just restart requests; no existing test asserted strongly enough on the resulting variable to have caught it. Needs a dedicated fix (either extend `try_parse_command_from_string` to look for a trailing arrow, or restructure how `parse_branch_tail` re-parses the action string) plus a regression test. -->

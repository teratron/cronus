...
**Based on:** .design/nodus/INDEX.md v1.0.64
**Status:** Active
...
<!-- Pending L2 Invariant-Compliance obligations (additive invariants on already-Done L1 concept specs; catalogued at INDEX v1.0.57, entries marked RESOLVED as they land — no running version stamp to re-drift) — awaiting L2 realization specs before they can be decomposed into verifiable atomic tasks:
  ...
  - NL-22 (compensation seam — declared compensating action, completed-effect ledger, LIFO reverse-completion-order unwind on scope failure/cancel/request, fallible-compensation surfaced as *incomplete* with the original effect left live, idempotent at-least-once, armed-not-automatic): the **heavier** of the two new obligations and the one with a genuine design fork. Verified at plan time: the crate has no compensation machinery at all, and NL-22 explicitly composes the LP-11 `decide → effect → observe` gate, the LP-17 settlement sink, and NL-12 suspend/resume for effects — **none of which exist in the crate**. Its spec pass must therefore decide whether to treat those compositions as vacuous-today host obligations and specify only the core contract (declare-a-compensation + completed-effect ledger + reverse-order driving + honest un-compensable recording), which is the Phase-15 precedent for streaming chunk-merge, or to sequence the underlying seams first. Needs a dedicated L2 realization spec before it can be phased.
  - NL-23 (bounded whole-run self-restart — run-grain `MAX:n` ceiling, visible carried restart count, run-boundary-only authority rejecting a request from inside `~FOR`/`~MAP`/`~PARALLEL`, fresh re-derivation from `@in`/`§config`, re-runs-does-not-undo): the **lighter and more tractable** of the two — self-contained in the executor's existing step loop, introduces no new value type (NL-7 untouched), and its NL-22 linkage is an honest-warning (rule e: a restart over non-idempotent, non-compensated effects double-commits), not a hard dependency. Blocker is narrow: an L2 spec fixing the directive's shape, where the restart count is exposed, how a "run-boundary step" is determined structurally, and the `NODUS:*` code for a ceiling-exceeded or nested-context request.
  ...
  - Invariant-Compliance traceability gap (found 2026-07-30 while verifying the tracking mechanism NL-22/NL-23 name): `l2-nodus-runtime.md` §3 Invariant Compliance lists only **NL-1…NL-10**, but thirteen further invariants exist (NL-11…NL-23), and each of their L1 Document History entries states that "l2-nodus-runtime carries NL-XX as a pending Invariant-Compliance obligation". It does not — there is no row. The obligations are tracked *only* in this Backlog. Not fixable from `magic.task` (spec bodies are `magic.spec`'s write domain); the next spec pass should reconcile §3 so the compliance table lists NL-11…NL-23 with explicit pending/realized status, making the stated carrier real rather than nominal. -->

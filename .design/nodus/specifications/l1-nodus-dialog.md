...
- **DG-11 Latest-reversible placement, and a decision-ready dialog payload**: [ADDED v1.4.0] two authoring properties DG-1…DG-10 leave entirely to the workflow author, both of which decide how much of a human's attention a run costs. **Placement**: where an `ASK`/`CONFIRM` position is the author's choice rather than a fixed control point, it belongs at the **latest still-reversible step** — every preparatory, undoable step runs first, so the human is asked **once, late, with the material assembled**, instead of repeatedly and early on questions the run itself would have settled. It never moves past a step whose effect is irreversible or outward (that is exactly where the confirmation must precede the effect), and never past a step whose behaviour genuinely depends on the answer, since deferring a question past the work it gates does not defer it — it answers it by assumption. **Payload**: the dialog presents a **decision-ready brief** — what the run produced, what the decision turns on, and what is being asked — carrying a **reference** to the produced artifact rather than the artifact's raw content inlined into the prompt. The artifact stays authoritative; the brief exists so the answer is fast, not so the artifact goes unread. Both are properties a validator MAY advise on, and neither alters the `Paused`/resume contract (DG-4) or the memoization semantics (DG-9).
...
| 1.4.0 | 2026-08-26 | Core Team | Amended — DG-11: **latest-reversible placement** and a **decision-ready payload** for author-positioned `ASK`/`CONFIRM` steps. Placement pushes the dialog to the last still-reversible step so the human is asked once, late, with the material assembled — bounded by never moving past an irreversible or outward effect, and never past a step whose behaviour depends on the answer (deferring a question past the work it gates answers it by assumption). Payload is a brief naming what the run produced, what the decision turns on, and what is asked, carrying a **reference** to the artifact rather than inlining its raw content; the artifact stays authoritative. Advisory to the validator; `Paused`/resume (DG-4) and memoization (DG-9) semantics unchanged. Nodus realization of the main review-checkpoint placement and brief rules. |

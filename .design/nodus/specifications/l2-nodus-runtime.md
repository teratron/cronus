...
| NL-26 A suspended external step declares its bound [ADDED v1.4.0] | **Partially realized.** The dialog case is discharged: DG-5's `+timeout` yields `DialogOutcome::Timeout` and a typed `NODUS:DIALOG_TIMEOUT` routed to `@err:` ([l2-nodus-dialog.md](l2-nodus-dialog.md) §3), which is precisely the behaviour NL-26 says a general external step must copy. The general case is vacuous behind a named upstream blocker — NL-12 generalized deferred execution is **Pending** above, so there is no non-dialog external step whose completion could fail to arrive. One piece is neither discharged nor vacuous, and it is the plannable residue: **the correlation token has no realization**. `ResumeDescriptor { workflow, vars, step_index }` carries no token and no deadline, and a resume is matched by workflow name plus step index alone, so a completion arriving after an elapse cannot be rejected as already-consumed — NL-26's invalidation rule has nothing to invalidate, and its no-reissue-without-compensation rule (NL-22) has no elapse event to hang on. It shares a struct, and therefore an implementation pass, with LP-22 (c)'s pinned generation ([l2-nodus-portability.md](l2-nodus-portability.md) §3.2); the two should land together rather than as two separate amendments to the same three-field published type. |

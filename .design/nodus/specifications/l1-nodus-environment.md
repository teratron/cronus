...
- [l1-nodus-authoring.md](l1-nodus-authoring.md) — [ADDED v1.5.1] **name collision kept deliberate**: NE's reproducible world is what a *workflow acts upon*, scored by a reward; NA's disposable world is where *nodus itself* runs while an author drives it, graded by nothing and existing only to keep runs uncontaminated. Different subjects and lifetimes; sharing a mechanism would hand one a reward it cannot use or strip the other of its grading.
...
- [../../main/specifications/l1-reasoning-spend.md](../../main/specifications/l1-reasoning-spend.md) — [ADDED v1.5.0] the main-side accounting contract NE-15 realizes at the evaluation substrate: billed-but-undelivered generation is a distinct class (RSN-1), unmeasured counts are declared lower bounds never zeros (RSN-3/RSN-10).
...
- **NE-15 Declared budget scope** [ADDED v1.5.0]: a token budget declared by a profile (NE-13) and denominated in an identified measure (NE-14) is **still ambiguous until the profile records which generated classes the budget counts**. The gap is not academic: a model that deliberates before answering produces, under one call, generation that is **billed and never delivered** and that is routinely **larger than the delivered answer**. Two hosts using the **same encoder** and the **same nominal budget**, one counting that class and one not, halt their candidates at spends that differ by a multiple — and the direction of the distortion is systematic, not noisy: a deliberation-heavy candidate is **advantaged** where the class is uncounted (it gets its thinking for free) and **penalized** where it is counted, so the same candidate wins or loses on a bookkeeping choice nobody declared. The profile therefore records the **budget scope** — the set of generated classes the count includes — and that scope is **part of the profile's identity** exactly as the budget (NE-13) and the measure (NE-14) are: an NE-12 archived tuple records it, rewards earned under different scopes are **not comparable**, and a host optimizer MUST partition its frontier by `(profile, budget, measure, scope)`, never by `(profile, budget, measure)` alone. An **undeclared or unresolvable** scope on a profile declaring a token budget is a **fail-fast pre-run rejection** (LP-8 kinship), never a silent default to "delivered output only" — the permissive reading is the one that hides the largest spend. Where the host's provider **does not report** an in-scope class, the run is halted on a **declared lower bound** and the NE-12 tuple records the count as incomplete; an estimate MUST NOT stand in for it, since an estimated count can halt the wrong candidate at the wrong point and produce a reward that looks valid and is not (NE-14 kinship). The class taxonomy, the counting, and the reporting are entirely **host-supplied** (LP-2 — the core names no provider and no class), enforcement remains the environment lifecycle's (NE-2) so a workflow can no more reinterpret its scope than extend its budget (LP-10 kinship), and a profile whose budget is wall-clock or step-count only carries no scope and is unaffected (**additive**). This is the nodus realization, at the evaluation substrate, of the main `l1-reasoning-spend` contract — billed-but-undelivered generation is a distinct accounting class (RSN-1), never conflated with delivered output, with unmeasured counts surfacing as declared lower bounds rather than zeros (RSN-3/RSN-10).
...
| --- | --- | --- | --- |
| 1.5.1 | 2026-09-11 | Core Team | Patch — cross-reference to `l1-nodus-authoring`, recording the deliberate two-worlds demarcation: NE's graded world is what a workflow acts upon, NA's disposable world is where nodus runs while an author drives it. Documentation linkage only; no invariant added or changed. |
| 1.5.0 | 2026-08-06 | Core Team | NE-15 added — **declared budget scope**, closing the twin of the NE-14 hole. NE-13 fixed the budget and NE-14 fixed the measure, but a token budget is still ambiguous until the profile says **which generated classes it counts**: a deliberating model produces, under one call, generation that is billed and never delivered and is routinely larger than the answer, so two hosts on the **same encoder** and the **same nominal budget** — one counting that class, one not — halt candidates at spends differing by a multiple, and the distortion is systematic rather than noisy (a deliberation-heavy candidate is advantaged where the class is uncounted and penalized where it is counted, winning or losing on an undeclared bookkeeping choice). Scope joins profile identity: NE-12 tuples record it, cross-scope rewards are incomparable, and the host optimizer partitions by `(profile, budget, measure, scope)`. An undeclared scope on a token-budgeted profile is a **fail-fast pre-run rejection**, never a silent default to delivered-output-only — the permissive reading hides the largest spend. Where a provider does not report an in-scope class the halt is on a **declared lower bound** with the tuple marked incomplete; estimates are barred, per NE-14. Host-supplied taxonomy and counting (LP-2), environment-enforced (NE-2), additive for wall-clock/step-only profiles. Nodus realization of main `l1-reasoning-spend` RSN-1/RSN-3/RSN-10. |

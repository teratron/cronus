...
### 3.2 Invariants added after v1.7.1 — realization status [ADDED v1.8.0]
...
**What is plannable, and what is not [ADDED v1.8.0].** None of the three is task-authorable
whole, and the reason differs in each case: LP-21 has no construct to attach to, while
LP-22 (c) and LP-23 each have one but carry an open type or LP-6 decision alongside it. Two
residues are genuinely small and independent of every blocker above — **LP-21's observable
binding** (one `RunManifest` field naming which implementation filled which role) and
**LP-22's pinned generation on `ResumeDescriptor`** (one optional, host-supplied, comparable
identity on a shipped struct). Both are additive, neither changes a trait signature, and each
is an honest win alone: the first makes an LP-8 claim inspectable, the second closes the only
LP-22 failure a crate with no import path can actually reach. They are the right first phase
if LP-21…LP-23 are opened at all; the version-line and degrade work is a separate and larger
design act, and the `fallback` degrade is blocked behind a mechanism the core does not have.

**Cross-spec note.** LP-22 (c)'s pinned generation and NL-26's correlation-token
invalidation (`l2-nodus-runtime.md` §3.1) are two fields on the *same* three-field
`ResumeDescriptor`. They should land in one pass rather than as two separate edits to the
same type — a second, independent amendment to a published struct is an avoidable LP-6 event.

...
| LP-17 Settlement effect seam | **Realized by** [l2-nodus-settlement.md](l2-nodus-settlement.md) [v1.0.0 Stable] — specified; awaiting implementation phase. Splits on LP-11's own decide/act seam: the decide half needs no design of its own (`EffectClass::Settlement` reuses `self.policy.evaluate("settlement", context)` unchanged — `context.args` already carries `[payee, amount, purpose]` positionally, per §4.9.2's existing raw-args shape); the act half needed a genuinely new `SettlementRail` trait, since `PolicyProvider::evaluate` returns only `bool` and has no channel for a receipt (VS-7) — the first LP-9…LP-20 item needing a new extension-point trait rather than reusing one already in the crate. VS-6's payment-required handshake stays explicitly out of nodus-core (host/workflow-authoring territory, not a new control-flow primitive). Not folded into this spec, mirroring the `l2-nodus-compensation`/`l2-nodus-restart` precedent: architecturally comparable in scope to a new provider trait, not a small `context`-extension like LP-16. |
...
**Leverage [UPDATED v1.7.0].** Of the twelve, two are **Implemented**: LP-11 (Phase 24,
v1.5.0) and LP-16 (Phase 25, v1.6.1); one is **specified, awaiting implementation**: LP-17
(`l2-nodus-settlement.md` v1.0.0, this pass) — the remaining nine split six vacuous
...
8. **Settlement effect seam (LP-17)** — specified in `l2-nodus-settlement.md` [v1.0.0]: a new `EffectClass::Settlement` + `SETTLE` command reuses this spec's own LP-11 gate unchanged for the decide half; a new `SettlementRail` trait + `NoopSettlementRail` built-in supplies the act half (`PolicyProvider::evaluate` has no receipt channel); `NODUS:SETTLEMENT_UNACCOUNTED` is `Signal`-free so `l2-nodus-error-dispatch.md`'s NL-9 mechanism picks it up automatically, with no dispatch-adjacent code needed here; `ExtensionRole::Settlement` extends the LP-8 manifest, deliberately absent from `HostCapabilities::builtin()` (`Storage`/`Policy`/`Dialog` precedent). Task-authorable — the next `/magic.task nodus` can open its implementation phase.

**[MODIFIED v1.7.0]** Item 8 added and immediately task-authorable — LP-17 moved from
*vacuous* to *specified* this pass. Items 2 and 7 remain **done**. Item 3's built-in half

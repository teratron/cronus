...
- **NG-7 Branch-exhaustiveness defers to the generated-value branch contract.** NG-3's **branch-exhaustiveness** check covers **author-written** control flow. Where a branch is decided by a **generated** value (a `GEN`/`REFINE` `Text`, a delegated external step's reply), the outcome-set obligation is owned entirely by **NL-25** (a closed, anchored, total outcome set routed to `@err:` on zero or multiple matches) — not NL-26, which governs a suspended external step's own completion deadline, an orthogonal concern for the same NL-12 step class; NG applies **no second exhaustiveness check** to that site — the branch contract already does, and double-jeopardy would train authors to suppress it. The deferral is scoped to *which arm*: NG-3's other checks are **not** deferred. **Loop-bound** in particular still applies — a `~UNTIL` whose continuation is decided by a generated value needs a `MAX:n` bound exactly as an author-written one does, and NL-25 says nothing about termination bounds; and NG-2's determinism, NG-4's reachability, and NG-5's impact all still project the generated-value site normally.
...
- **NG-10 Export is a pure projection.** The graph exports to a standard graph-interchange format for the automation canvas (`l1-automation-canvas`) and external tools, carrying each edge's verb (NG-1). Syntax-extracted edges and inferred edges (a resolved generated-value branch outcome per NL-25) are **distinguishable** in the export (the provenance distinction). The export is a one-way projection, never a second source of truth, never an import path back into the definition.
...
| Alias | Path | Purpose |
| --- | --- | --- |
| `[LANG]` | `.design/nodus/specifications/l1-nodus-language.md` | The projected language; NL-25 alone owns generated-value branch-exhaustiveness (NG-7), not loop-bound and not NL-26 (a suspended external step's own deadline — unrelated) |
...
| --- | --- | --- | --- |
| 1.1.0 | 2026-09-12 | Core Team | **Second review, `RFC → Stable`** (`/magic.spec nodus`, per this spec's own v1.0.0-RFC entry naming "a second review" as the promotion bar). **Council Verdict** — Agreement: NG-1…NG-11 remain internally consistent; the projection/analysis boundary (NG-2/NG-3) and the host boundary (NG-11) hold up under a fresh adversarial read; cross-workspace citations to `l1-code-intelligence` §6 and `l1-derived-artifact-handoff` DAH-1 verified accurate by direct inspection. Clashes: none structural. Blind spot found and closed — a genuine citation defect the first review's own correction did not fully resolve: that review narrowed NG-7 to say "NL-25 owns only *which arm* (outcome-set totality), not termination bounds" but left **five** sites (Related Specifications, NG-3, NG-7 itself twice, NG-10, the `[LANG]` Canonical Reference) still citing **"NL-25/NL-26"** jointly as the outcome-set owner. `NL-26` ("A suspended external step declares its bound") governs a **deadline for a suspended external step** — an orthogonal NL-12-adjacent concern with no "closed, anchored, total outcome set" mechanism of its own; it was never the second half of NL-25's contract, and its inclusion made every one of those five citations name a spec that does not do what was claimed of it. Recommendation: correct all five to name NL-25 alone, noting NL-26's actual (unrelated) scope where a reader might otherwise wonder why it was dropped. First Action: applied inline before promotion, not deferred — matching this spec's own NG-2 discipline that a claim traceable to the wrong source is worse than an absent one. Unreciprocated cross-workspace citations (`l1-change-merge`, `l1-automation-canvas`, `l1-requirement-checklists` name no `l1-nodus-graph` back-reference) checked and left as-is — soft references are advisory and non-mutual by this project's own Circular Guard rule (`spec.md`), and `l1-change-merge` §4's generic step/macro delta-granularity note already accommodates NG-5/NG-6 without needing to name this spec specifically; correcting the other direction would mean editing main-workspace files from a nodus-scoped pass, out of C15's workspace-scope isolation. `[DR]` Promoted to Stable on this pass rather than deferred to an `l2-nodus-graph` authoring pass — Trust Mode (C9) criteria all clear (no RULES.md conflict, no hard-dependency cycle, L1 layer constraints n/a, MVC satisfied well past the minimum) and the spec's own bar ("a second review **or** an authoring pass") is disjunctive, not conjunctive. (Override: `/magic.spec nodus amend l1-nodus-graph` to revisit.) No invariant's substance changed — NG-1…NG-11 read exactly as before except for the corrected attribution. |

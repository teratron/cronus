...
- [l1-nodus-language.md](l1-nodus-language.md) — the language whose definition this spec projects: `@steps:` sequence, `→pipeline` / `→ $name` data flow, `@ctx:` fields, `RUN(@macro)`, `ROUTE(wf:name)`, `?IF`/`?SWITCH` branches, `~FOR`/`~UNTIL` loops, `@err:` dispatch, `~compensate` (NL-22). NG-7 defers the generated-value branch case entirely to NL-25/NL-26.
- [l1-nodus-testing.md](l1-nodus-testing.md) — NT-10 route-coverage advisory (`W006`); NG-4 generalizes it from "a routed target with no test" to "a step with no path from entry".
...
2. **Blast radius is guessed.** A change to one step's output shape breaks a downstream consumer three steps away, and the author finds out at run time because nothing computed the reachable set.
...
- **NG-3 Model-free structural analysis family.** From the graph alone, with no model, the subsystem derives: **step reachability / dead-step** detection (NG-4); **impact / blast-radius** — the forward closure from a changed step over `pipes` + `writes`→`reads` + `calls`, depth-bounded, each hit tagged with depth and the edge it arrived by, classified by change kind (NG-5); **loop-bound** check — a `~UNTIL` reachable without a `MAX:n` bound on its `loops` edge; **branch-exhaustiveness** — a `?SWITCH` with no `*` default over a value whose domain is not closed (NG-7 scopes this); **macro call graph** with hot-path ranking (macros by transitive `calls`-in count) and **god-macro** ranking (a macro referenced far above the median); **context-field lifecycle** — a `@ctx:` field `read` with no prior `write` on any path (use-before-def), or `write` with no `read` (dead write); and a **dependency-cycle** check over the `calls` graph (a load-time validation error, not an advisory). Traversals are depth-limited to stay bounded on large or recursive workflows (NL-18).

- **NG-4 Dead-step detection generalizes route coverage.** A step with **no path from the workflow entry under any combination of branch outcomes** is a dead step — the workflow analog of unreferenced code, and the generalization of NT-10's route-coverage advisory from "a `ROUTE(wf:x)` target with no exercising `@test:`" to "a step the control-flow graph cannot reach". It is emitted at **warning severity** with a diagnostic code, is **advisory** (it does not block parse or execution, matching NT-10), and names the step by index and label.

- **NG-5 Impact is classified by change kind.** A change to a step is classified as a **contract change** — its `→ $name` output shape, the `@ctx:` fields it `writes`, its argument arity — or a **body-only change**. A contract change propagates along `pipes` and `writes`→`reads` to every downstream consumer; a body-only change usually has an **empty** structural blast radius. This is the high-signal / low-signal split, and it is the input the delta model in `l1-change-merge` consumes at step / macro granularity.
...
- **NG-7 Structural checks defer to the generated-value branch contract.** NG-3's loop-bound and branch-exhaustiveness checks cover **author-written** control flow. Where a branch or a loop-continuation is decided by a **generated** value (a `GEN`/`REFINE` `Text`, a delegated external step's reply), the outcome-contract obligation is owned entirely by NL-25/NL-26 (a closed, anchored, total outcome set routed to `@err:` on zero or multiple matches); NG applies **no second mechanism** to that site and reports no exhaustiveness finding there — the branch contract already does, and double-jeopardy would train authors to suppress the check.
...
- **Call-graph cycle** — an SCC over `calls` is a load-time validation error (not advisory).
...
diff_blast(wf_before, wf_after) -> {
  ...
  summary:         "step 4 GEN output shape changed — reaches steps 7, 9 and macro @score",
}
```
...
- **Alternative — fold these checks into the validator (`l1-nodus-language`).** Rejected for the advisory family: a dead step or a dead context write is a smell, not an error, and NT-10 already establishes that coverage-style findings are advisory (`W00x`), not parse failures. The one hard case — a macro call-graph cycle — *is* a validation error and is stated as such (NG-3).
...
| Alias | Path | Purpose |
| --- | --- | --- |
| `[LANG]` | `.design/nodus/specifications/l1-nodus-language.md` | The projected language; NL-25/NL-26 own the generated-value branch case (NG-7) |
| `[TESTING]` | `.design/nodus/specifications/l1-nodus-testing.md` | NT-10 route-coverage advisory that NG-4 generalizes |

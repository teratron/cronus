...
- [../../main/specifications/l1-environment-lifecycle.md](../../main/specifications/l1-environment-lifecycle.md) — [ADDED v1.11.0] the host-side lifetime contract for the ephemeral environment a workflow's effectful steps run in; LP-18 is the workflow-side rule that its disappearance is a typed failure, never a hang or a silent substitution
- [../../main/specifications/l1-staged-rollout.md](../../main/specifications/l1-staged-rollout.md) — [ADDED v1.12.0] the host-side release contract LP-19 reads from: the host computes the exposure decision, nodus only defines how the resolved value enters and behaves inside a run (resolved once, frozen, fail-closed, read-only)
- [../../main/specifications/l1-convergence-gate.md](../../main/specifications/l1-convergence-gate.md) — [ADDED v1.13.0] the host-side placement contract for a deterministic gate at a convergence boundary; LP-20 is the workflow-side realization of its deferred-obligation half (CG-5/CG-6), an effect gated on a host-tracked precondition
- [../../main/specifications/l1-scoped-generalization.md](../../main/specifications/l1-scoped-generalization.md) — [ADDED v1.14.3] LP-3 is the **prior worked instance** of that contract's SG-3: a pattern observed in one host enters the library only on evidence from **two independent** host contexts, with §4.14's admission record making the judgment falsifiable. The main-side spec states the same rule once for every learned artifact — where LP-3 governs what enters a portable library, SG governs how far any learned pattern may be applied. LP-3 is unchanged and remains the authority for this library's admission.
- [../../main/specifications/l1-corpus-originality.md](../../main/specifications/l1-corpus-originality.md) — [ADDED v1.14.2] the host-side admission gate §4.4 points at: a host schema layer that accumulates near-duplicate commands loses discriminating power for NL-16 `@needs` disclosure and any description-based routing (the ORI-11 cost model). Policy and thresholds stay host-side; nodus contributes only the LP-12 seam where the finding is computed.
- [../../main/specifications/l1-value-settlement.md](../../main/specifications/l1-value-settlement.md) — LP-17 is the workflow-side realization of the main outbound value-settlement contract: a settlement is a distinct effect class gated against a host-held spending envelope, the rail host-supplied
...
- **LP-15 Host-supplied durable-state seam**: [ADDED v1.8.0] durable workflow state — state a workflow persists **across invocations** (a paused-run resume descriptor NL-12/DG-4, memoized approvals DG-9, any workflow-declared data meant to outlive one run) — is reached through the host-supplied **`StorageProvider`** role (§4.1, the TBD now resolved), so a workflow **never assumes where its durable state lives**. The core ships one built-in **in-memory** default (non-durable, discarded between invocations, sufficient for in-process testing), and a host MAY back the seam with a **local on-device store or a remote database** with no change to the workflow: a workflow that reads and writes durable state is **portable across both** (LP-3), because it targets the interface, not a backend. The nodus core names **no database, path, transport, or auth** (LP-1/LP-2); *where* state is stored, and whether persisting it leaves the device, are **host concerns** — moving state off-device is the host's egress decision, never something nodus performs or assumes. A workflow that declares durable state as an LP-8 capability is rejected fail-fast on a host that provides no `StorageProvider` (the in-memory default satisfies only non-durable needs), so a durable-state need surfaces before the run, not mid-run. This is the nodus realization of the main `l1-deployment-neutrality` provider-seam contract — the user-data/storage plane as an abstract seam (DN-2) whose default is local and whose remote backing is a host choice (DN-3), a workflow's state portable across backends with the local default always valid (DN-7) — the host-neutral analog, at the workflow-library layer, of the host keeping identity/auth/storage swappable without a server dependency.

- **LP-16 Effect risk-class declaration**: [ADDED v1.9.0] an effectful step (LP-11: model-call / tool-use / deferred) MAY declare, alongside its effect class, a set of **consequence descriptors** — reversibility, external-visibility, and value-at-stake magnitude — so a host **graduated authorization gate** can **tier the effect by its consequence** (auto / confirm / approval) rather than gating every effect uniformly. The runtime carries the declaration to the host `PolicyProvider` (LP-11) **before** the effect, preserving the decide → effect → observe ordering. The **tier-to-friction mapping, the thresholds, and the gate strength are entirely host-supplied** (LP-2 — the nodus core names no risk metric, no tier, and no friction level); and — per LP-10 — a step **declares** its consequence but can **never set or lower its own gate tier** (a workflow cannot self-authorize a high-consequence effect as auto). An **unclassified** effect defaults to the host's most-cautious handling (**fail-safe-to-friction**), never the least. Purely additive: a step that declares no consequence descriptors is gated exactly as LP-11 today — the `PolicyProvider` decides with whatever it has, and a host with no gate runs as today. This is the nodus realization of the main `l1-action-gating` risk-proportional contract — its explicit consequence axes (AG-2, the declared descriptors), its proportional tiering (AG-1/AG-3, the host maps them to friction), its fail-safe-to-friction default (AG-4), and its declare-not-self-set discipline (AG-6/AG-8, the host owns the tier and its floor) — the workflow-side declaration that lets a host spend approval friction only where consequence warrants, complementing the static LP-8 manifest (can this effect happen at all?) and the dynamic LP-11 gate (is this use permitted?) with a *how-much-friction* dimension.

- **LP-17 Settlement effect seam**: [ADDED v1.10.0] an effectful step MAY declare itself a **settlement** — an outbound transfer of value to a named recipient (a paid host tool/API, or a charging external peer via LP-14) — carrying a **value-transfer descriptor** (recipient + amount + purpose) alongside the LP-16 consequence descriptors. A settlement is gated by the host `PolicyProvider` (LP-11, **decide → settle → observe**) against a **host-held spending envelope** — per-payment ceiling, cumulative ceiling, allowlisted recipients, rate limit — that the workflow **declares a need for but can never set, raise, or relax** (LP-10): the envelope lives on the host authority plane, and a step may read a host-exposed remaining figure but cannot widen it. The gate is **fail-closed with a settlement-specific discipline**: a *deny*, an exhausted envelope, a non-allowlisted recipient, an over-ceiling amount, or an unreachable/erroring rail aborts the settlement with a typed `NODUS:*` code and **the paid step does not proceed** — the runtime never silently falls back to an unpaid/unmetered path, never retries the payment outside the authorized bound, and never loops a renegotiated price (a price beyond the authorized amount aborts, not re-pays). The **rail is entirely host-supplied** (LP-2): the nodus core names no currency, wallet, account, ledger, or payment protocol; it contributes only the *declare-a-settlement-effect-and-let-the-host-gate-and-settle-it* seam. The settlement result enters the run as **untrusted provenance** (NL-11) and MAY carry a host-supplied **execution receipt** (HO-9 witness) binding recipient + amount + time so a fabricated or unaccounted settlement is distinguishable from a real one. A workflow declaring a settlement effect is rejected **fail-fast pre-run** on a host that provides no settlement rail (LP-8 kinship — a settlement capability unmet is a missing capability, surfaced before the first step, never mid-run); a workflow that settles nothing is unaffected (**additive**, no rail = no settlement possible = today's behaviour). This is the *outbound-value* specialization of the LP-11 effect gate (LP-11 gates any effect; LP-17 adds the payee/envelope/allowlist/no-unmetered-fallback semantics a value transfer needs) and reuses LP-14's verified peer as the peer-payee case; it is the nodus realization of the new main `l1-value-settlement` contract — its distinct-outbound-concern (VS-1), principal-set agent-read-only envelope (VS-2, via LP-10 + the host authority plane), fail-closed-never-unmetered rule (VS-5), bounded machine-negotiable handshake (VS-6), receipted accountability (VS-7, via HO-9), and opt-in rail-neutral seam (VS-8, via LP-2 + LP-8) — while the funding source, the rail, the currency, and the envelope storage stay host-side (LP-1/LP-2), the workflow holding only a bounded, declared, host-gated ability to request a transfer it can never self-authorize.
- **LP-18 Environment-liveness seam**: [ADDED v1.11.0] where a host supplies an **ephemeral execution environment** for a workflow's effectful steps (tool use, command execution, filesystem access), that environment's availability is a declared capability (LP-8 — a workflow needing an environment on a host that provides none is rejected **pre-run**, never mid-step), and its **disappearance mid-run is a typed fail-fast failure — never a hang, never a silent substitution**. Three rules carry the weight. (a) **A vanished environment fails typed, immediately**: a step whose environment becomes unavailable (destroyed, reaped, evicted, or lost) aborts with a typed `NODUS:*` code naming the environment identity, routing to `@err:` or halting per the workflow's own error handling — the runtime MUST NOT block indefinitely waiting on an environment that is gone, and MUST NOT quietly retry the step into a *different* environment, because a step that silently re-runs somewhere else has lost every precondition its predecessors established. (b) **A resume into a missing environment is a failed resume, not a fresh start**: a suspended run (NL-12/DG-4) whose environment no longer exists fails its resume with a typed code rather than resuming into a newly-provisioned environment that merely *looks* the same — telling a workflow it resumed when its accumulated state is gone is the most damaging available lie, since every subsequent step reasons from a world that no longer holds. (c) **Nodus captures nothing**: preserving what an ending environment held is entirely host-side (LP-1/LP-2 — the core names no image, container, mount, path, or snapshot); the runtime's sole contribution is that each step event records the environment identity it ran against (HO-2 attribution, HO-7 correlation), so a host can correlate a residue it captured to the exact steps that produced it. Purely **additive**: a workflow declaring no environment need, or running on a host with a single always-present environment, behaves exactly as today. This is the workflow-side realization of the main `l1-environment-lifecycle` contract — its closed status set with *failed* distinct from *gone* (EL-2), its same-instance-or-report-a-failed-resume rule (EL-7), and its every-exit-path-names-a-capture-owner discipline (EL-5), whose capture side stays wholly with the host.

- **LP-19 Host-supplied exposure switch seam**: [ADDED v1.12.0] a workflow MAY read a **named exposure switch** whose value is supplied entirely by the host (a staged behaviour change, an experiment arm, a reserved holdout). Nodus computes no assignment, holds no rollout state, and names no fraction, hash function, subject, bucketing key, or targeting rule (LP-2 — the core carries no rollout vocabulary whatsoever); it defines only how a value the host already decided **enters and behaves inside a run**, and three rules make that behaviour safe. (a) **Resolved once, frozen for the run**: every switch a workflow reads is resolved at run start and is immutable for the run's lifetime — a mid-run re-resolution is forbidden, so a run can never straddle two variants with half its steps on each side, which would make its trace attributable to neither arm and quietly poison any comparison built on it (the runtime-side guarantee behind the manifest record HO-18 writes). A suspended run (NL-12/DG-4) resumes under the values it started with, not under whatever the host would decide now. (b) **Fail-closed to a declared safe value**: a switch the host cannot resolve — unknown name, unavailable ruleset, stale beyond the host's bound — yields the switch's **declared safe value** (normally the unchanged behaviour), never an error that halts the run and never an implicit *on*; an undeclared safe value is a pre-run validation failure in the LP-8 family, surfaced before the first step rather than mid-run. (c) **Read-only, ordinary, provenance-carrying**: a switch value enters as an ordinary value with configuration provenance (NL-11/NL-20 kinship — being a switch confers no trust), and — per **LP-10** — a workflow reads its switches but can never set, widen, flip, or self-assign one; requesting an arm is not a vocabulary the language has. Purely **additive**: a workflow that reads no switch behaves exactly as today, and a host that resolves none supplies every switch's safe value. This is the workflow-side realization of the new main `l1-staged-rollout` contract — its deterministic host-computed assignment (SR-2), its fail-closed local evaluation (SR-9), and the non-straddling requirement that makes a per-run arm attribution meaningful (SR-11 via HO-18).

- **LP-20 Obligation-gated effect seam**: [ADDED v1.13.0] an effectful step MAY declare its effect **gated on a named obligation** — a host-tracked precondition that some prior event must have discharged — and the runtime guarantees the host's obligation check runs **before** the effect, reusing the LP-11 `decide → effect → observe` ordering: an **undischarged** obligation aborts the effect with a typed `NODUS:*` (fail-closed, never a silent proceed and never a hang). The obligation is the workflow-side carrier of a *deferred* control — one step's action (adding a dependency, producing an artifact) records that a later, dependent effect (installing it, shipping it) is now blocked until a required action (a scan, a review) has genuinely completed. Three rules bind it. **Discharge is recognized only on genuine completion**, never on an attempt: a step that errored, or a differently-shaped step that merely resembles the required action, does not clear the obligation — the host owns what counts as discharge (LP-2), and the runtime only guarantees the gate consults it before the effect. **Per LP-10 a workflow declares the dependency but never marks its own obligation discharged**: requesting to be un-gated is not a vocabulary the language has, so a workflow assembled under untrusted influence (NL-11) cannot self-satisfy the precondition it is gated on. And **an obligation left open at run end is surfaced, not dropped**: the terminal `RunManifest` records outstanding obligations (composing HO-10 completeness honesty), because a silently forgotten obligation is a policy that quietly did not apply, indistinguishable from one satisfied. The obligation namespace, its tracking store, and the discharge predicate are **entirely host-supplied** (LP-2 — the core names no obligation, scanner, or policy); nodus contributes only the declare-a-gate-and-let-the-host-resolve-it seam and the ordering guarantee. The dynamic *precondition* complement to the LP-11 per-effect gate (LP-11 asks “is this effect allowed now?”, LP-20 asks “has the required prior action happened?”) and to the static LP-8 manifest; purely **additive** — a workflow declaring no obligation gate, or a host tracking none, behaves exactly as today. This is the nodus realization of the new main `l1-convergence-gate` deferred-obligation contract (CG-5 gated-on-genuine-discharge / CG-6 outstanding-obligations-surfaced), the workflow-side half of a control the host places at a convergence boundary.
...
| Storage | `StorageProvider` | Persist and retrieve durable workflow state across invocations; host-supplied backend, local or remote, off-device only by host egress decision (LP-15, §4.11) | Built-in *in-memory* (non-durable; state discarded between invocations) |
...
| Vocabulary | `SchemaProvider` | Extend the builtin command vocabulary and reserved-variable set with host-declared names | `BuiltinSchemaProvider` (no extensions; pure builtin vocabulary) |
| Dialog | `DialogProvider` | Pose a typed question or approval decision to a human and return the outcome (see `l1-nodus-dialog.md`) | `DefaultDialogProvider` (resolves from the step's `+default` modifier, otherwise signals `Pause`; performs no I/O and never blocks, so non-interactive and test runs stay deterministic) |
| Environment | `EnvironmentProvider` | Supply a graded-run task world over a closed reset/step/evaluate lifecycle (NE-1, see `l1-nodus-environment.md`) | `StubEnvironment` (one deterministic in-process environment for testing) |
| Config | `ConfigProvider` | Decide host acceptance of a `§config` candidate that already passed the pure shape check (NL-20, see `l1-nodus-language.md`) | `DefaultConfigProvider` (accepts the shape-checked candidate as-is; no I/O, no store, no UI — the `NoopPolicyProvider` LP-2 built-in precedent) |
| Settlement | `SettlementRail` | Execute an outbound value settlement under host authority (LP-17, §4.13) | `NoopSettlementRail` (settles nothing and returns no receipt, so a workflow declaring settlement runs without effect until a host supplies a rail) |

> The table above is the authoritative extension point registry: an implementation that introduces a role not listed here must amend this spec. **[MODIFIED v1.14.4]**
>
> **On LP-3 and this table.** Earlier text read "new roles are added only when LP-3 is satisfied", which the table itself does not bear out: `Storage` is a listed role whose LP-3 disposition §4.8.2 records as **not satisfied** (one integration context, not two independent decision shapes). Listing a role and satisfying LP-3 for it are therefore *different* facts, and conflating them makes the registry look like it certifies something it does not. The accurate reading: **LP-3 gates whether a host-driven capability is distilled into the library** (§4.2 criteria) — a role may be listed here, shipped, and built-in-satisfiable while its LP-3 admission record remains open, and §4.8 is where each seam's disposition is tracked. A row in this table asserts that the role exists and what its built-in does, nothing more.
>
> **Registry-vs-reality discipline (why this note now says how, not just what).** Between v1.0 and v1.14.3 this table listed four roles while the library shipped nine — `Vocabulary`, `Dialog`, `Environment`, `Config`, and `Settlement` each entered `ExtensionRole` through a build phase without the amendment this note requires, and `Config` reached the point of having no mention anywhere in this document at all. The rule did not fail because anyone disagreed with it; it failed because nothing *read* it at the moment a role was added. Two consequences worth stating plainly rather than re-asserting the rule and hoping:
>
> - **This table is a projection of `ExtensionRole`, and drift between them is a defect in this spec, not in the code.** The enum is what LP-8 capability manifests are expressed in (§4.5), so a role missing here leaves the manifest contract's own vocabulary undefined for that role — the registry stops being able to describe what a workflow may legitimately require.
> - **The check belongs where a role is minted.** Adding an `ExtensionRole` variant is the moment this table is stale; any later audit is discovering the drift rather than preventing it. A conformance check that asserts the two agree is the enforceable form of this note, and is the standing follow-up that would let the sentence above stop relying on memory.
...
**Near-duplicate accumulation in the host layer.** [ADDED v1.14.2] LP-4 keeps host commands *out of* the baseline; nothing keeps a host layer from accumulating **near-duplicates of its own entries** — two commands whose descriptions differ only in a swapped domain noun. Each is individually well-formed and passes every validator, so the layer grows in count while its vocabulary loses discriminating power. The damage lands on **selection**: NL-16 `@needs` selective disclosure and any host-side routing over command descriptions have no principled basis to choose between two entries that describe the same operation, so a correct query resolves arbitrarily. This is the routing-ambiguity cost model of the main `l1-corpus-originality` contract (ORI-11), and its admission gate — comparison against the whole layer **and** the pending batch, over neutralized content, against a published calibrated baseline — is the shape a host schema layer wants as its vocabulary grows. Nodus supplies the seam rather than the policy: a host's LP-12 vetting classifier is where such a finding is computed, since the corpus being protected is the **host's** layer and the thresholds are a property of *that* corpus. The nodus core names no similarity measure and no threshold, and a host supplying no classifier is unaffected.
...
### 4.11 Host-Supplied Durable-State Seam (LP-15)

The `StorageProvider` role (§4.1) is a *persist/retrieve* seam over durable state; it is the storage analog of the model/audit/policy seams — one abstract interface, one built-in default, backend host-supplied:

```text
[REFERENCE]
trait StorageProvider:
    put(key, bytes)            // persist durable state
    get(key) -> bytes?          // retrieve; None if absent
    delete(key)

builtin InMemoryStore : StorageProvider   // non-durable, in-process, no I/O — the default

persist_state(run, host):
    if run.needs_durable_state and not host.provides_durable(StorageProvider):
        reject_fail_fast("workflow requires durable state; host provides none")   // LP-8, pre-run
    host.storage.put(run.state_key, serialize(run.durable_state))   // where this lands is host's choice
```

Two properties keep it host-neutral. First, **location-agnostic**: `put`/`get` name a key and bytes — never a path, DSN, table, or URL — so a host may implement them over a local file store, an embedded DB, or a remote database, and the workflow is byte-for-byte identical across all three (LP-3). Second, **egress stays host-side**: whether `put` writes to disk or ships bytes to a remote backend is the host's decision and, if remote, the host's egress-consent concern (mirroring the main DN-4 gate) — nodus neither performs nor assumes off-device persistence. A host wiring only `InMemoryStore` runs exactly as today (additive); a durable-state need is declared (LP-8) and satisfied by a host that provides a durable backend, or rejected fail-fast before the run.

### 4.12 Effect Risk-Class Declaration Seam (LP-16)

LP-16 adds an optional descriptor to the LP-11 effect gate — the step declares *consequence*, the host maps consequence to *friction*:

```text
[REFERENCE]
effect := {
  class        : model_call | tool_use | deferred    // LP-11
  reversible?  : bool                                  // consequence descriptors (LP-16) — optional
  external?    : bool                                  // crosses the trust boundary
  value?       : none | money | creds | perms          // value at stake
}
...
Two properties hold. First, **the workflow never sets its own tier** (LP-10): it states *what the effect does* (reversible? external? value?), and the host decides *how much friction* — a step cannot declare itself `auto`. Second, **additive and host-neutral**: no descriptors and no `PolicyProvider` means today's behaviour; the core carries no tier, threshold, or friction vocabulary (LP-2). It is the *how-much-friction* complement to LP-8 (may this effect exist?) and LP-11 (is this use allowed?).

### 4.13 Settlement Effect Seam (LP-17)

LP-17 specializes the LP-11 effect gate for an **outbound value transfer**: the step declares a *settlement* (recipient + amount + purpose), and the host gates it against a spending envelope the workflow can read but never set (LP-10), settling through a host-supplied rail the core never names (LP-2).

```text
[REFERENCE]
settlement := {
  class     : settlement                              // specializes the LP-11 effect classes
  payee     : recipient id (a host tool/API, or an LP-14 peer)
  amount    : value to transfer                        // consequence: value-at-stake (LP-16)
  purpose   : what it pays for
}
...
| --- | --- | --- | --- |
| 1.14.4 | 2026-08-12 | Core Team | **§4.1's authoritative extension-point registry listed four roles while the library ships nine** — found by a `/magic.spec nodus` Creative-Spark pass (DA-3: highest-coverage gap) reading the Backlog's long-standing "L1 §4.1 taxonomy reconciliation" note, which recorded the count as 4-vs-8; grounding it against `crates/nodus/src/portability.rs` showed nine. Added the five missing rows with their real interface names and built-ins, verified in source rather than inferred: `Vocabulary`/`SchemaProvider`/`BuiltinSchemaProvider`, `Dialog`/`DialogProvider`/`DefaultDialogProvider` (resolves from `+default`, else `Pause`; no I/O, never blocks), `Environment`/`EnvironmentProvider`/`StubEnvironment`, `Config`/`ConfigProvider`/`DefaultConfigProvider` (accepts the shape-checked candidate as-is), `Settlement`/`SettlementRail`/`NoopSettlementRail` (settles nothing, returns no receipt). Post-Update Review caught one layer-purity slip in the first draft and it was fixed before promotion: the Config row cited `l2-nodus-config` as its authority, an L1 reaching down to an L2 for a contract an L1 already owns — `§config` acceptance is NL-20 in `l1-nodus-language`, which is what the row now cites. **`Config` was the sharpest case: it had zero mentions anywhere in this document**, so a shipped extension role was entirely absent from the spec that calls itself its registry. The §4.1 note is reworded to say *how* the rule holds rather than only restating it, because the rule did not fail through disagreement — it failed because nothing read it at the moment a role was minted, five times across the phases that added Dialog, Environment, Config, and Settlement. Two consequences recorded: the table is a **projection of `ExtensionRole`**, so drift is a defect in this spec rather than in the code; and since LP-8 manifests are expressed in that enum (§4.5), a role missing here leaves the manifest contract's vocabulary undefined for it. A conformance check asserting table-vs-enum agreement is named as the enforceable form and the standing follow-up. **Stays Stable, patch level, deliberately:** every added row points at a contract already Stable elsewhere (`l1-nodus-dialog`, `l1-nodus-environment` NE-1, `l1-nodus-language` NL-20 for `§config`, and this document's own §4.13 for Settlement) and every role is already shipped and tested, so no implementation gains an obligation it did not already have — the amendment rule's RFC-revert is for *substantive new requirements*, and this is an index catching up to its own document. This is the `l2-crate-topology` 1.1.1 completeness-fix precedent (a missing table row for an already-defined invariant, patch, no status change), applied at five rows instead of one. **A second review finding, from the instruction-quality pass:** the note's inherited clause "new roles are added only when LP-3 is satisfied" is contradicted by the very table it governs — `Storage` is listed while §4.8.2 records its LP-3 disposition as *not satisfied*. Rather than restate a rule the document falsifies on its own next page, §4.1 now separates the two facts: listing a role asserts that it exists and what its built-in does; LP-3 gates **distillation** (§4.2) and each seam's admission record lives in §4.8. Left unfixed, the amendment would have propagated a contradiction into the one table every conformance reader treats as authoritative. |
| 1.14.3 | 2026-08-06 | Core Team | Related Specifications record `l1-scoped-generalization` — LP-3's two-independent-host admission rule is the **prior worked instance** of that spec's SG-3 (recurrence within one context evidences that a pattern is real, never that it generalizes; only independent contexts license breadth). The main side had the rule only here, written for library extraction, and now states it once for every learned artifact. Link-only; LP-3 unchanged and still the authority for this library's admission. |
| 1.14.2 | 2026-08-06 | Core Team | §4.4 records **near-duplicate accumulation in the host schema layer**: LP-4 keeps host commands out of the built-in baseline but nothing keeps a host layer from accumulating near-copies of its own entries, each individually well-formed and validator-clean. The cost is **selection** — NL-16 `@needs` disclosure and host-side routing over command descriptions have no principled basis to choose between two entries describing the same operation, so a correct query resolves arbitrarily (the ORI-11 routing-ambiguity cost model of the main `l1-corpus-originality` contract). Nodus supplies the **seam, not the policy**: the finding is computed by the host's LP-12 vetting classifier, since the protected corpus is the host's layer and the thresholds are a property of that corpus; the core names no similarity measure and no threshold, and a host with no classifier is unaffected. Documentation-only; no invariant added or changed. |
...
| 1.10.0 | 2026-07-17 | Core Team | Added LP-17 (settlement effect seam) — an effectful step MAY declare itself a settlement (outbound value transfer to a named recipient — a paid host tool/API or a charging LP-14 peer) carrying a value-transfer descriptor (recipient + amount + purpose) alongside the LP-16 consequence descriptors; gated by the host PolicyProvider (LP-11, decide → settle → observe) against a host-held spending envelope (per-payment ceiling, cumulative ceiling, allowlisted recipients, rate limit) the workflow declares a need for but can never set/raise/relax (LP-10), reading a host-exposed remaining figure at most; fail-closed with settlement discipline — deny / exhausted envelope / non-allowlisted recipient / over-ceiling / unreachable rail aborts with a typed NODUS:* and the paid step does not proceed, never a silent unpaid/unmetered fallback, never a retry outside the authorized bound, never a renegotiated-price loop (VS-5/VS-6); rail entirely host-supplied (LP-2, no currency/wallet/ledger/protocol in core); settlement result enters untrusted (NL-11) optionally carrying an HO-9 receipt so a fabricated/unaccounted settlement is detectable (VS-7); a declared settlement is rejected fail-fast pre-run on a host with no rail (LP-8 kinship), a workflow that settles nothing unaffected (additive); new §4.13. The outbound-value specialization of the LP-11 effect gate (adds payee/envelope/allowlist/no-unmetered-fallback over the generic gate), reusing LP-14's verified peer as the peer-payee case. The nodus realization of the new main l1-value-settlement contract (VS-1 distinct outbound concern / VS-2 principal-set agent-read-only envelope via LP-10 / VS-5 fail-closed-never-unmetered / VS-6 bounded handshake / VS-7 receipted accountability via HO-9 / VS-8 opt-in rail-neutral seam via LP-2 + LP-8) — funding source, rail, currency, and envelope storage stay host-side. L1 stays Stable (C9); l2-nodus-portability carries LP-17 as a pending Invariant-Compliance obligation reconciled at magic.task (LP-8…LP-16 precedent). |
| 1.9.0 | 2026-07-09 | Core Team | Added LP-16 (effect risk-class declaration) — an effectful step (LP-11: model-call / tool-use / deferred) MAY declare consequence descriptors (reversibility, external-visibility, value-at-stake magnitude) alongside its effect class, so a host graduated authorization gate can tier the effect by consequence (auto / confirm / approval) rather than gating uniformly; the runtime carries the declaration to the host `PolicyProvider` before the effect (decide → effect → observe preserved); tier-to-friction mapping + thresholds + gate strength entirely host-supplied (LP-2, no risk metric/tier/friction in core), and — per LP-10 — a step declares its consequence but can never set or lower its own tier; an unclassified effect defaults to the host's most-cautious handling (fail-safe-to-friction); purely additive (no descriptors + no PolicyProvider = today's behaviour); new §4.12. The how-much-friction complement to the static LP-8 manifest (may this effect exist?) and the dynamic LP-11 gate (is this use allowed?). The nodus realization of the new main l1-action-gating risk-proportional contract (AG-1/AG-2 consequence axes + proportional tiering / AG-4 fail-safe-to-friction / AG-6 & AG-8 declare-not-self-set, host owns the tier + floor). L1 stays Stable (C9); l2-nodus-portability carries LP-16 as a pending Invariant-Compliance obligation reconciled at magic.task (LP-8…LP-15 precedent). |
| 1.8.0 | 2026-07-09 | Core Team | Added LP-15 (host-supplied durable-state seam) — durable workflow state (state persisted across invocations: a paused-run resume descriptor NL-12/DG-4, memoized approvals DG-9, any workflow-declared outlive-one-run data) is reached through the host-supplied `StorageProvider` role (§4.1 TBD now resolved), so a workflow never assumes where its durable state lives; the core ships one built-in in-memory default (non-durable, discarded between invocations) and a host MAY back the seam with a local on-device store or a remote database with no workflow change (portable across both, LP-3); the core names no database/path/transport/auth (LP-1/LP-2) — where state is stored and whether persisting it leaves the device are host concerns (off-device is the host's egress decision, mirroring main DN-4), never something nodus performs or assumes; a workflow declaring durable state as an LP-8 capability is rejected fail-fast on a host with no durable `StorageProvider` (the in-memory default satisfies only non-durable needs), so a durable-state need surfaces pre-run; §4.1 StorageProvider row filled + new §4.11. The nodus realization of the new main l1-deployment-neutrality provider-seam contract (DN-2 storage plane as abstract seam / DN-3 local-or-remote by host choice / DN-7 portability across backends) — the workflow-library analog of the host keeping identity/auth/storage swappable without a server dependency. L1 stays Stable (C9); l2-nodus-portability carries LP-15 as a pending Invariant-Compliance obligation reconciled at magic.task (LP-8…LP-14 precedent). |

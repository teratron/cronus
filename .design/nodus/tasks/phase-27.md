...
- **Notes:** Reconcile `l2-nodus-settlement.md`'s `[REFERENCE]` pseudocode to the exact
  as-built code once T-27A01–T-27C02 land (the Phase-17/20/23/24/25/26 Track-C precedent —
  spec pseudocode is illustrative, not binding on exact identifier names, but should match
  reality once it exists). `l2-nodus-portability.md` §3.1's LP-17 row moves from "Realized by
  `l2-nodus-settlement.md` — specified; awaiting implementation" to Implemented; its Leverage
  paragraph and §5 item 8 update to Done. `INDEX.md` rows + top-level version synced for every
  spec file actually touched.
